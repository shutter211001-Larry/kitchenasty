...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || t("erp_747"));
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={isForgotPassword ? handleForgotPassword : handleSubmit} className="w-full space-y-5">
          ...
          <div className="space-y-1.5 relative group">
            ...
            <div className="relative">
              ...
              <input type="email" placeholder={t("erp_750")} value={email} onChange={e => setEmail(e.target.value)} className="w-full bg-white/[0.04] border border-white/[0.08] focus:border-primary/50 text-white rounded-2xl py-3.5 pl-12 pr-4 text-sm font-semibold placeholder:text-slate-600 outline-none transition-all focus:ring-4 focus:ring-primary/10" required />
            </div>
          </div>
