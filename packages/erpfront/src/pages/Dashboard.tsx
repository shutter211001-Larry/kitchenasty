...
const formatRelativeTime = (dateString: string) => {
  ...
  if (diffSec < 60) {
    return i18n.t("erp_297");
  } else if (diffMin < 60) {
    return `${diffMin} 分鐘前`;
  } else if (diffHr < 24) {
    return `${diffHr} 小時前`;
  } else if (diffDay === 1) {
    return i18n.t("erp_298");
  } else if (diffDay < 7) {
    return `${diffDay} 天前`;
  } else {
                ...
                {stats.lowStockIngredients.map(ing => <div key={ing.id} className="flex justify-between items-center bg-white border border-border p-3.5 rounded-xl shadow-sm hover:shadow-md transition-all group">
                    ...
                    <div className="flex items-center gap-6">
                      <div className="text-right">
                        ...
                        <p className="font-mono text-xs font-black text-red-500 mt-0.5">
              ...
              {stats.recentPriceUpdates && stats.recentPriceUpdates.length > 0 ? stats.recentPriceUpdates.slice(0, 10).map((up, idx) => <div key={up.id} className="flex gap-4 items-start">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground mt-0.5">
                        {`已更新 ${up.ingredientName}${up.packageSize === 1 && (up.packageUnit.toLowerCase() === "kg" || up.packageUnit.toLowerCase() === "l") ? "" : ` ${up.packageSize}${up.packageUnit.toUpperCase()}`} 報價: $${up.price}`}
                      </p>
