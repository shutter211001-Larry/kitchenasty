  ...
  return (
    <div className="flex flex-col gap-8">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">供應商管理</h2>
          <p className="text-muted-foreground mt-1">管理配合廠商聯絡資訊、合約與原物料採購報價紀錄</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {loading ? (
          <div className="col-span-full py-20 text-center">
            ...
            <span className="text-sm font-medium text-muted-foreground">正在載入供應商資料...</span>
          </div>
          ...
          <div className="col-span-full py-20 text-center text-muted-foreground border-2 border-dashed border-border rounded-3xl bg-white/50">
            ...
            <p className="font-medium">找不到符合的供應商資料，請點擊右上角新增。</p>
          </div>
        ) : (
          filteredSuppliers.map((s) => (
            <div key={s.id} className="bg-white p-6 sm:p-8 rounded-[2rem] border border-border shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between min-h-[360px] relative group">
              <div>
                ...
                <h3 className="text-2xl font-bold mb-1 text-gray-800">{s.name}</h3>
                <p className="text-sm text-muted-foreground mb-6 font-medium">
                  {s.contactPerson ? `聯絡窗口: ${s.contactPerson}` : '暫無登錄窗口'}
                </p>
                ...
                <div className="space-y-4 pt-6 border-t border-border">
                  <div className="flex items-center gap-3 text-sm text-gray-600">
                    ...
                    <span className="font-semibold text-gray-700">{s.phone || '未提供電話'}</span>
                  </div>
                  <div className="flex items-center gap-3 text-sm text-gray-600">
                    <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center shrink-0">
                      <MapPin className="w-4 h-4 text-muted-foreground" />
                    </div>
                    <span className="truncate font-semibold text-gray-700" title={s.address || '未提供地址'}>
                      {s.address || '未提供地址'}
                    </span>
                  </div>
                </div>
