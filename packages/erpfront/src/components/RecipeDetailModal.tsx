      ...
      <div className="bg-white rounded-[2.5rem] w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col animate-in fade-in zoom-in duration-200">
        <header className="p-8 pb-6 border-b border-border bg-muted/20 relative">
          ...
          <div className="flex flex-wrap gap-4 mt-6">
            ...
            {isPortionUnit(recipe.yieldUnit) && recipe.yieldAmount > 0 && <div className="flex items-center gap-2 bg-emerald-50 text-emerald-700 px-4 py-2 rounded-xl border border-emerald-100 shadow-sm font-bold text-sm">
            ...
            {recipe.bakingLossRate > 0 && <div className="flex items-center gap-2 bg-orange-50 text-orange-700 px-4 py-2 rounded-xl border border-orange-100 shadow-sm font-bold text-sm">
        ...
        <div className="flex-1 overflow-y-auto p-8 flex gap-8">
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="space-y-4">
              {(recipe.steps || []).map((step: any, index: number) => <div key={step.id} className="relative pl-8 pb-6 border-l-2 border-primary/20 last:pb-0">
                  ...
                  <div className="bg-muted/30 rounded-2xl p-5 border border-border/50">
                    <div className="flex justify-between items-center mb-3">
                      ...
                      {step.parameters && step.parameters.length > 0 && <div className="flex flex-wrap gap-2">
                          {step.parameters.map((p: any, pIdx: number) => <div key={pIdx} className="flex items-center gap-1 text-[10px] font-bold text-muted-foreground bg-white/50 px-2 py-0.5 rounded-full border border-border/50">
                              <Clock className="w-3 h-3" />
                              {p.value} {p.unit}
                            </div>)}
                        </div>}
                    </div>
                    {step.description && <p className="text-sm text-gray-600 mb-4 font-medium">
                        {step.description}
                      </p>}

                    {/* Ingredients in this step (Collapsible) */}
                    {step.items && step.items.length > 0 && <div className="mt-4 pt-4 border-t border-border/50">
                        <div className="flex justify-between items-center mb-3">
                          <span className="text-xs font-bold text-muted-foreground flex items-center gap-1">
                            {t("erp_211")}
                            {step.items.length})
                          </span>
                          <button onClick={() => toggleStep(step.id)} className="p-1 hover:bg-white rounded text-muted-foreground transition-all">
                            {collapsedSteps.has(step.id) ? <ChevronDown className="w-4 h-4" /> : <ChevronUp className="w-4 h-4" />}
                          </button>
                        </div>

                        {!collapsedSteps.has(step.id) && <div className="space-y-2 animate-in slide-in-from-top-2 duration-200">
                            {step.items.map((item: any) => <div key={item.id} className="flex justify-between items-center bg-white/60 p-2 px-3 rounded-xl border border-border/30 text-xs hover:border-primary/30 transition-all">
                                <span className="font-bold text-gray-700 flex items-center gap-2">
                                  {item.sourceStepId ? `步驟 ${recipe.steps.findIndex((s: any) => s.id === item.sourceStepId) + 1} (${item.sourceStep?.action || i18n.t("erp_212")})` : item.ingredient?.name || item.subRecipe?.name || i18n.t("erp_213")}
                                </span>
          ...
          <div className="w-80 space-y-6">
            ...
            {recipe.totalIngredients && recipe.totalIngredients.length > 0 && <div className="bg-muted/20 rounded-[2rem] p-6 border border-border mt-6">
                <h4 className="text-xs font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2 mb-4">
                  <PackageOpen className="w-4 h-4" />
                  {t("erp_220")}
                </h4>
                <div className="space-y-3">
                  {recipe.totalIngredients.map((ing: any) => <div key={ing.id} className="flex justify-between items-center text-sm border-b border-border/50 pb-2 last:border-0 last:pb-0">
