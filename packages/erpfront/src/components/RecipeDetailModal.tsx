...
import { X, Clock, Utensils, PieChart, ChefHat, ChevronUp, ChevronDown, PackageOpen, Flame } from 'lucide-react';
...
const RecipeDetailModal = ({ recipe, onClose }: Props) => {
  const [collapsedSteps, setCollapsedSteps] = React.useState<Set<string>>(new Set());
      ...
      <div className="bg-white rounded-[2.5rem] w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col animate-in fade-in zoom-in duration-200">
        <header className="p-8 pb-6 border-b border-border bg-muted/20 relative">
          <div className="flex justify-between items-start">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h3 className="text-3xl font-black text-gray-800 tracking-tight">{recipe.name}</h3>
              </div>
              <p className="text-sm text-muted-foreground">{recipe.description || '無食譜描述'}</p>
            </div>
          ...
          <div className="flex flex-wrap gap-4 mt-6">
            <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl border border-border shadow-sm">
              ...
              <span className="text-sm font-bold">{recipe.calories?.toFixed(0) || 0} kcal</span>
            </div>
            <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl border border-border shadow-sm">
              <Utensils className="w-4 h-4 text-orange-500" />
              <span className="text-sm font-bold">成本 ${recipe.cost?.toFixed(1) || 0}</span>
            </div>
            {isPortionUnit(recipe.yieldUnit) && recipe.yieldAmount > 0 && (
              <div className="flex items-center gap-2 bg-emerald-50 text-emerald-700 px-4 py-2 rounded-xl border border-emerald-100 shadow-sm font-bold text-sm">
                ...
                <span>單個成本 ${(recipe.cost / recipe.yieldAmount).toFixed(1)}/{recipe.yieldUnit}</span>
              </div>
            )}
            {recipe.bakingLossRate > 0 && (
              <div className="flex items-center gap-2 bg-orange-50 text-orange-700 px-4 py-2 rounded-xl border border-orange-100 shadow-sm font-bold text-sm">
                ...
                <span>烤焙損耗率 {recipe.bakingLossRate}%</span>
              </div>
            )}
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-8 flex gap-8">
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="space-y-4">
              {(recipe.steps || []).map((step: any, index: number) => (
                <div key={step.id} className="relative pl-8 pb-6 border-l-2 border-primary/20 last:pb-0">
                  ...
                  <div className="bg-muted/30 rounded-2xl p-5 border border-border/50">
                      ...
                      {step.parameters && step.parameters.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                          {step.parameters.map((p: any, pIdx: number) => (
                            <div key={pIdx} className="flex items-center gap-1 text-[10px] font-bold text-muted-foreground bg-white/50 px-2 py-0.5 rounded-full border border-border/50">
                    ...
                    {step.description && <p className="text-sm text-gray-600 mb-4 font-medium">{step.description}</p>}
                      ...
                      <div className="mt-4 pt-4 border-t border-border/50">
                        <div className="flex justify-between items-center mb-3">
                          <span className="text-xs font-bold text-muted-foreground flex items-center gap-1">
                            材料清單 ({step.items.length})
                          </span>
                          <button 
                            onClick={() => toggleStep(step.id)}
                            className="p-1 hover:bg-white rounded text-muted-foreground transition-all"
                          >
                            {collapsedSteps.has(step.id) ? <ChevronDown className="w-4 h-4" /> : <ChevronUp className="w-4 h-4" />}
                          </button>
                        </div>
                        ...
                        {!collapsedSteps.has(step.id) && (
                          <div className="space-y-2 animate-in slide-in-from-top-2 duration-200">
                            {step.items.map((item: any) => (
                              <div key={item.id} className="flex justify-between items-center bg-white/60 p-2 px-3 rounded-xl border border-border/30 text-xs hover:border-primary/30 transition-all">
                                <span className="font-bold text-gray-700 flex items-center gap-2">
                                  {item.sourceStepId 
                                    ? `步驟 ${(recipe.steps.findIndex((s:any) => s.id === item.sourceStepId) + 1)} (${item.sourceStep?.action || '產出'})` 
                                    : item.ingredient?.name || item.subRecipe?.name || '(未知)'}
                                </span>
                                <span className="text-muted-foreground font-mono bg-white px-2 py-0.5 rounded shadow-sm border border-border/50">
                                  {item.portionQuantity ? `${item.portionQuantity} 份` : `${item.quantity} g`}
                                </span>
                              </div>
                            ))}
                          </div>
          ...
          <div className="w-80 space-y-6">
            ...
            <NutritionRadar data={recipe} title="營養佔比雷達圖" />
            
            <div className="bg-muted/20 rounded-[2rem] p-6 space-y-4 border border-border">
              <div className="flex justify-between items-center text-sm">
                <span className="text-muted-foreground font-bold">蛋白質</span>
                <span className="font-mono font-black">{recipe.protein?.toFixed(1) || 0} g</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-muted-foreground font-bold">脂肪</span>
                <span className="font-mono font-black">{recipe.fat?.toFixed(1) || 0} g</span>
              </div>
              <div className="flex justify-between items-center text-sm pl-4 border-l-2 border-border/60">
                <span className="text-muted-foreground text-xs font-semibold">├─ 飽和脂肪</span>
                <span className="font-mono text-xs font-bold">{recipe.saturatedFat?.toFixed(1) || 0} g</span>
              </div>
              <div className="flex justify-between items-center text-sm pl-4 border-l-2 border-border/60">
                <span className="text-muted-foreground text-xs font-semibold">└─ 反式脂肪</span>
                <span className="font-mono text-xs font-bold">{recipe.transFat?.toFixed(1) || 0} g</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-muted-foreground font-bold">碳水化合物</span>
                <span className="font-mono font-black">{recipe.carbohydrates?.toFixed(1) || 0} g</span>
              </div>
              <div className="flex justify-between items-center text-sm pl-4 border-l-2 border-border/60">
                <span className="text-muted-foreground text-xs font-semibold">└─ 糖</span>
                <span className="font-mono text-xs font-bold">{recipe.sugar?.toFixed(1) || 0} g</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-muted-foreground font-bold">鈉</span>
                <span className="font-mono font-black">{recipe.sodium?.toFixed(0) || 0} mg</span>
              </div>
            </div>
              ...
              <div className="bg-muted/20 rounded-[2rem] p-6 border border-border mt-6">
                ...
                <div className="space-y-3">
                  {recipe.totalIngredients.map((ing: any) => (
                    <div key={ing.id} className="flex justify-between items-center text-sm border-b border-border/50 pb-2 last:border-0 last:pb-0">
        ...
        <footer className="p-8 border-t border-border bg-muted/20 flex justify-end">
          <button onClick={onClose} className="px-10 py-4 bg-gray-800 text-white rounded-2xl font-black shadow-xl hover:scale-105 active:scale-95 transition-all">
            關閉視窗
          </button>
        </footer>
      </div>
