...
const UnitSelector = ({
  ...
  useEffect(() => {
    const clickOutside = (e: MouseEvent) => {
      if (isOpen && buttonRef.current && !buttonRef.current.contains(e.target as Node) && dropdownRef.current && !dropdownRef.current.contains(e.target as Node)) {
        setIsOpen(false);
      }
    };
  ...
  return <div className={cn("relative inline-block", className)}>
      <button ref={buttonRef} type="button" onClick={toggleDropdown} className="px-2 py-1 bg-muted/65 hover:bg-muted border border-border/50 rounded-lg text-[10px] font-bold text-gray-700 flex items-center gap-1 shadow-sm transition-all">
        <span>{value}</span>
        <ChevronRight className="w-2.5 h-2.5 rotate-90 opacity-60" />
      </button>

      {isOpen && createPortal(<div ref={dropdownRef} id="unit-selector-dropdown" style={{
      position: "absolute",
      top: `${coords.top}px`,
      left: `${coords.left}px`,
      zIndex: 9999
            ...
            {activeGroup === null ? <div className="space-y-0.5">
                <div className="px-2 py-1 text-[9px] font-black text-muted-foreground uppercase tracking-widest">
                  {t("erp_13")}
                </div>
                {availableGroups.map(g => <button key={g.id} type="button" onClick={e => {
          e.stopPropagation();
          setActiveGroup(g);
          ...
          setActiveGroup(null);
                ...
                {activeGroup.units.map((u: any) => <button key={u.id} type="button" onClick={e => {
          e.stopPropagation();
          onChange(u.name);
          setIsOpen(false);
        }} className={cn("w-full text-left px-2 py-1.5 text-[11px] font-bold rounded-lg transition-all flex justify-between items-center", value === u.name ? "bg-primary/5 text-primary" : "text-gray-700 hover:bg-muted/50 hover:text-primary")}>
                    <span>{u.name}</span>
...
const CreateRecipeModal = ({
  ...
  const [yieldUnit, setYieldUnit] = useState(initialData?.yieldUnit || t("erp_15"));
  ...
  useEffect(() => {
    ...
    const fetchResults = async () => {
      try {
        ...
        const recipes = recRes.data.filter((r: any) => r.name.toLowerCase().includes(search.toLowerCase())).map((r: any) => ({
          ...r,
          type: "recipe"
        }));
  ...
  const calculateStepOutput = (stepId: string, currentSteps: any[], visited: Set<string> = new Set()): {
    totalWeight: number;
  ...
  const addStep = (actionName: string) => {
    ...
    for (const group of actionGroups) {
      ...
      if (actionObj) {
        const mergedUnitGroupIds = new Set([...(group.defaultUnitGroups || []).map((ug: any) => ug.id), ...(actionObj.defaultUnitGroups || []).map((ug: any) => ug.id)]);
    ...
    if (source.droppableId.startsWith("library-")) {
      ...
      if (itemData.isAction && destination.droppableId === "steps") {
        ...
        const newStep = {
          ...
          parameters: [{
            value: 0,
            unit: itemData.action === t("erp_18") ? "℃" : itemData.action === t("erp_19") ? "cm" : "min"
          }],
          items: []
        };
        const newSteps = Array.from(steps);
  ...
  return <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4 bg-black/60 backdrop-blur-md">
      <div className="bg-white rounded-t-[2rem] md:rounded-[2.5rem] w-full md:max-w-5xl h-[95vh] md:h-auto md:max-h-[85vh] overflow-hidden shadow-2xl flex flex-col animate-in fade-in slide-in-from-bottom-10 md:slide-in-from-bottom-0 md:zoom-in duration-200">
              ...
              <div className="space-y-4 md:space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="col-span-1 md:col-span-2 space-y-2">
                    ...
                    <input type="text" placeholder={t("erp_27")} className="w-full px-4 py-3 bg-white border border-border rounded-xl outline-none focus:ring-4 focus:ring-primary/10 transition-all text-base font-bold" value={name} onChange={e => setName(e.target.value)} required />
                  </div>
                  <div className="col-span-1 space-y-2">
                    ...
                    <div className="flex gap-2">
                      <input type="number" className="flex-1 px-4 py-3 bg-white border border-border rounded-xl outline-none focus:ring-4 focus:ring-primary/10 transition-all text-base font-bold" value={yieldAmount} onChange={e => setYieldAmount(parseFloat(e.target.value) || 1)} min="1" />
                      <select className="w-24 px-2 py-3 bg-white border border-border rounded-xl outline-none focus:ring-4 focus:ring-primary/10 font-bold text-sm" value={yieldUnit} onChange={e => setYieldUnit(e.target.value)}>
                        {unitGroups.filter((g: any) => [i18n.t("erp_29"), i18n.t("erp_30"), i18n.t("erp_31")].includes(g.name)).map((g: any) => <optgroup key={g.id} label={g.name}>
                              {g.units.map((u: any) => <option key={u.id} value={u.name}>
                                  {u.name}
                                </option>)}
                            </optgroup>)}
                      </select>
                  ...
                  <div className="col-span-1 space-y-2">
                    ...
                    <input type="number" placeholder={t("erp_33")} className="w-full px-4 py-3 bg-white border border-border rounded-xl outline-none focus:ring-4 focus:ring-primary/10 transition-all text-base font-bold" value={bakingLossRate || ""} onChange={e => setBakingLossRate(Math.max(0, Math.min(99, parseFloat(e.target.value) || 0)))} min="0" max="99" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-primary/5 border border-primary/10 px-5 py-3.5 rounded-2xl flex items-center justify-between shadow-sm animate-in fade-in slide-in-from-top-1 duration-150">
                    <label className="flex items-center gap-3 cursor-pointer select-none w-full">
                      <input type="checkbox" checked={isSubRecipe} onChange={e => {
                  ...
                  <div className="bg-primary/5 border border-primary/10 px-5 py-3.5 rounded-2xl flex items-center justify-between shadow-sm animate-in fade-in slide-in-from-top-1 duration-150">
                    <label className="flex items-center gap-3 cursor-pointer select-none w-full">
                      <input type="checkbox" checked={isProduct} onChange={e => {
                ...
                <div className="bg-white border border-border rounded-2xl p-6 shadow-sm space-y-4">
                  <div className="flex justify-between items-center border-b border-border pb-3">
                    ...
                    <button type="button" onClick={() => setOutputs([...outputs, {
                    type: "PRIMARY",
                    name: "",
                    yield: 0,
                    unit: "g",
                  ...
                  {outputs.map((out, idx) => <div key={idx} className="flex flex-col md:flex-row gap-2 md:items-center bg-muted/20 p-3 md:p-2 rounded-lg border border-border">
                      <select value={out.type} onChange={e => {
                  ...
                  {/* AI Nutrition Button */}
                  <div className="pt-2">
                    <button type="button" onClick={handleCalculateAiNutrition} disabled={loading || outputs.length === 0 || steps.length === 0} className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-indigo-50 border border-indigo-200 text-indigo-700 font-bold rounded-xl hover:bg-indigo-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                      <span>{t("erp_46")}</span>
                    </button>
                    {outputs.some(o => o.nutritionFacts) && <div className="mt-4 space-y-2 text-xs">
                        <p className="font-bold text-gray-700">{t("erp_47")}</p>
                        <div className="grid grid-cols-2 gap-2">
                          {outputs.map((out, idx) => out.nutritionFacts ? <div key={idx} className="bg-white p-3 rounded border border-gray-200">
                ...
                <Droppable droppableId="steps" type="step">
                  {provided => <div {...provided.droppableProps} ref={provided.innerRef} className="space-y-6">
                      {steps.length === 0 ? <div className="py-20 border-2 border-dashed border-border rounded-[2rem] text-center text-muted-foreground flex flex-col items-center gap-4 bg-white/50">
                          ...
                          <p className="font-bold">{t("erp_52")}</p>
                            ...
                            {provided => <div ref={provided.innerRef} {...provided.draggableProps} onClick={() => setActiveStepId(step.id)} className={cn("bg-white border rounded-[2rem] shadow-sm hover:shadow-md transition-all overflow-hidden cursor-pointer", activeStepId === step.id ? "border-primary ring-2 ring-primary/20 shadow-lg" : "border-border")}>
                                  ...
                                  <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex-1 md:w-auto">
                                      <div className="flex flex-wrap items-center gap-2">
                                        ...
                                        <input type="text" className="font-bold text-sm bg-transparent outline-none border-b border-transparent focus:border-primary min-w-[120px] flex-1" placeholder={t("erp_53")} value={step.description} onChange={e => setSteps(steps.map(s => s.id === step.id ? {
                                ...s,
                            ...
                            removeStep(step.id);
                                  ...
                                  <div className="flex flex-col md:flex-row md:items-center gap-2 md:ml-auto">
                                    {index > 0 && <div className="flex gap-1 mr-2">
                                        {steps.slice(0, index).map((prevStep, psIdx) => {
                              const remaining = calculateRemainingOutput(prevStep.id, steps);
                              if (remaining <= 0) return null;
                              return <button key={prevStep.id} onClick={e => {
                                e.stopPropagation();
                                addStepOutputToStep(step.id, prevStep);
                              }} className="px-2 py-1 bg-muted hover:bg-gray-800 hover:text-white rounded text-[10px] font-bold transition-all" title={`加入步驟 ${psIdx + 1} (${prevStep.action}) 的產出 (剩餘 ${remaining}%)`}>
                                                {t("erp_54")}
                                                {psIdx + 1}
                                              </button>;
                            })}
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                      {step.parameters.map((param: any, pIdx: number) => <div key={pIdx} className="flex items-center bg-muted rounded-lg p-0.5 border border-border/50">
                                            <input type="number" className="w-12 bg-transparent px-1 py-1 text-xs text-right font-bold outline-none" value={param.value} onChange={e => {
                                ...
                                setSteps(steps.map(s => s.id === step.id ? {
                                  ...
                                  parameters: newParams
                                            ...
                                            <UnitSelector className="ml-1" value={param.unit} onChange={val => {
                                const newParams = [...step.parameters];
                                ...
                                return unitGroups.filter(g => !usedGroupIds.includes(g.id));
                                            ...
                                            {step.parameters.length > 1 && <button onClick={e => {
                                e.stopPropagation();
                            ...
                            removeStep(step.id);
                                ...
                                <Droppable droppableId={step.id} type="item">
                                  {provided => <div {...provided.droppableProps} ref={provided.innerRef} className="p-4 bg-white/50 min-h-[50px] space-y-2">
                                      {step.items.length === 0 ? <div className="py-4 text-center text-[10px] text-muted-foreground border border-dashed border-border rounded-xl italic">
                                          {t("erp_55")}
                                        </div> : step.items.map((item: any, iIdx: number) => <Draggable key={item.id} draggableId={item.id} index={iIdx}>
                                              {(itemProvided, itemSnapshot) => <div ref={itemProvided.innerRef} {...itemProvided.draggableProps} className={cn("flex flex-col bg-white p-3 rounded-xl border border-border shadow-sm group transition-shadow w-full", itemSnapshot.isDragging && "shadow-lg ring-2 ring-primary/20")}>
                                                  <div className="flex items-center gap-3 w-full">
                                                    ...
                                                    <div className={cn("w-2 h-2 rounded-full shrink-0", item.type === "ingredient" ? "bg-primary" : item.type === "recipe" ? "bg-orange-500" : "bg-gray-800")} />
                                                    ...
                                                    <div className="flex items-center gap-2 shrink-0">
                                                      {item.type === "step_output" ? <div className="flex items-center gap-1.5 mr-2">
                                                          <input type="number" min="0" max="100" className="w-14 bg-muted px-1.5 py-1 text-xs text-right font-bold rounded-lg border border-border/50 outline-none focus:border-primary transition-colors" value={item.quantity} onChange={e => {
                                      const newQty = parseFloat(e.target.value) || 0;
                                      setSteps(steps.map(s => s.id === step.id ? {
                                        ...s,
                                        items: s.items.map((it: any) => it.id === item.id ? {
                                          ...it,
                                          quantity: newQty
                                        } : it)
                                      } : s));
                                                          ...
                                                          {item.type === "recipe" && <div className="flex bg-muted rounded-lg p-0.5">
                                                              <button type="button" onClick={() => setSteps(steps.map(s => s.id === step.id ? {
                                        ...s,
                                        items: s.items.map((it: any) => it.id === item.id ? {
                                          ...it,
                                          usePortion: false
                                        } : it)
                                      } : s))} className={cn("px-1.5 py-0.5 text-[8px] font-bold rounded", !item.usePortion ? "bg-white shadow-sm text-primary" : "text-muted-foreground")}>
                                                                {t("erp_56")}
                                                              </button>
                                                              <button type="button" onClick={() => setSteps(steps.map(s => s.id === step.id ? {
                                        ...s,
                                        items: s.items.map((it: any) => it.id === item.id ? {
                                          ...
                                          usePortion: true
                                                          ...
                                                          <input type="number" className="w-16 px-1.5 py-1 bg-muted rounded text-right font-mono font-bold text-xs outline-none" value={item.usePortion ? item.portionQuantity : item.quantity} onChange={e => {
                                      const val = parseFloat(e.target.value) || 0;
                                      setSteps(steps.map(s => {
                                        if (s.id === step.id) {
                                          return {
                                            ...s,
                                            items: s.items.map((it: any) => it.id === item.id ? item.usePortion ? {
                                              ...it,
                                              portionQuantity: val
                                            } : {
                                              ...it,
                                        ...
                                        return s;
                                                          ...
                                                          {item.usePortion ? <span className="text-[10px] font-bold text-muted-foreground w-6">
                                                              {t("erp_15")}
                                                            </span> : <UnitSelector value={item.unit || "g"} onChange={val => {
                                      setSteps(steps.map(s => {
                                        if (s.id === step.id) {
                                          return {
                                            ...s,
                                    ...
                                    }} availableGroups={unitGroups.filter((g: any) => [i18n.t("erp_29"), i18n.t("erp_30"), i18n.t("erp_31")].includes(g.name))} />}
                                                        </div>}
                                                      <button type="button" onClick={() => removeItemFromStep(step.id, item.id)} className="opacity-0 group-hover:opacity-100 p-1 text-muted-foreground hover:text-destructive transition-opacity">
                                ...
                                const ingList = Object.entries(sourceOut.ingredients).map(([name, qty]) => `${name} ${(qty * ratio).toFixed(1)}g`).join(", ");
                                return <div className="mt-1.5 pl-6 flex flex-col gap-1 text-[10px] text-gray-500 font-bold self-start animate-in slide-in-from-top-1 duration-100">
                                                          <span>
                                                            {t("erp_57")}
                                                            {usedWeight.toFixed(1)}
                                                            {t("erp_58")}
                                                          </span>
                                                          {ingList && <span className="text-gray-400">
                                                              {t("erp_59")}
                                                              {ingList}
                                                            </span>}
                                                        </div>;
                                ...
                                const conv = (ingredient.unitConversions || []).find((c: any) => c.fromUnit === itemUnit && c.toUnit === baseUnit);
                                const revConv = (ingredient.unitConversions || []).find((c: any) => c.fromUnit === baseUnit && c.toUnit === itemUnit);
                                if (conv) {
                                  const convertedQty = (item.quantity * conv.multiplier).toFixed(1);
                                  return <div className="mt-1.5 pl-6 flex items-center gap-1 text-[10px] text-emerald-600 font-bold bg-emerald-50/50 py-0.5 px-2 rounded-md border border-emerald-100/50 w-fit self-start animate-in slide-in-from-top-1 duration-100">
                                                            <span>
                                                              ✓ {item.quantity}{" "}
                                                              {itemUnit} ≈{" "}
                                                              {convertedQty}{" "}
                                  ...
                                  return <div className="mt-1.5 pl-6 flex items-center gap-1.5 text-[10px] text-amber-600 font-black bg-amber-50/80 py-1 px-2.5 rounded-md border border-amber-100 w-fit self-start animate-in slide-in-from-top-1 duration-100">
            ...
            <div className="flex-1 bg-white p-4 md:p-8 overflow-y-auto md:border-l border-border order-1 md:order-2 h-[45vh] md:h-auto shrink-0 md:shrink border-b md:border-b-0 shadow-sm md:shadow-none z-10">
              <div className="space-y-6 md:space-y-10">
                ...
                <section className="space-y-4">
                  ...
                  return <div key={group.name} className="border border-border rounded-2xl overflow-hidden shadow-sm">
                        <button onClick={() => toggleGroup(group.name)} className="w-full flex items-center justify-between p-4 bg-muted/10 hover:bg-muted/20 transition-all">
                        ...
                        {isOpen && <Droppable droppableId={`library-actions-${group.name}`} isDropDisabled={true} type="step">
                            {provided => <div {...provided.droppableProps} ref={provided.innerRef} className="p-3 grid grid-cols-2 gap-2 bg-white animate-in slide-in-from-top-2 duration-200">
                                {group.actions.map((actionObj: any, idx: number) => <Draggable key={actionObj.id} draggableId={JSON.stringify({
                          isAction: true,
                          action: actionObj.name
                        })} index={idx}>
                                      {(provided, snapshot) => <>
                                          <div ref={provided.innerRef} {...provided.draggableProps} className={cn("bg-muted/30 hover:bg-white rounded-xl border border-transparent hover:border-primary transition-all overflow-hidden", snapshot.isDragging && "opacity-50 ring-2 ring-primary")}>
                                            <div className="flex items-center">
                                              ...
                                              <button onClick={() => addStep(actionObj.name)} className="flex-1 py-3 px-3 text-left text-xs font-bold text-gray-700 hover:text-primary transition-all flex justify-between items-center group">
                                                {actionObj.name}
                                                <Plus className="w-3 h-3 opacity-0 group-hover:opacity-100" />
                                              </button>
                                            </div>
                                          </div>
                                          {snapshot.isDragging && <div className="py-3 px-3 bg-primary text-white rounded-xl text-xs font-bold flex justify-between items-center">
                                              {actionObj.name}
                                              <Plus className="w-3 h-3" />
                                            </div>}
                                        </>}
                ...
                <section className="space-y-4">
                  ...
                  <div className="space-y-4">
                    <div className="relative">
                      ...
                      <input type="text" placeholder={t("erp_64")} className="w-full pl-10 pr-4 py-3 bg-muted/40 border border-border rounded-xl outline-none focus:ring-4 focus:ring-primary/10 transition-all text-sm" value={search} onChange={e => setSearch(e.target.value)} />
                    </div>
                    ...
                    <Droppable droppableId="library-ingredients" isDropDisabled={true} type="item">
                      {provided => <div {...provided.droppableProps} ref={provided.innerRef} className="space-y-2">
                          {searchResults.map((item, idx) => <Draggable key={item.id} draggableId={JSON.stringify({
                        ...item,
                        isAction: false
                      })} index={idx}>
                              {(provided, snapshot) => <>
                                  <div ref={provided.innerRef} {...provided.draggableProps} className={cn("bg-white rounded-xl border border-border shadow-sm transition-all overflow-hidden", snapshot.isDragging && "opacity-50")}>
                                    <div className="flex items-center">
                                      <div {...provided.dragHandleProps} className="p-3 text-muted-foreground/30 hover:text-primary cursor-grab active:cursor-grabbing border-r border-border/50">
                                        <GripVertical className="w-3.5 h-3.5" />
                                      </div>
                                      <button onClick={() => activeStepId && addIngredientToStep(activeStepId, item)} className="flex-1 text-left p-3 hover:bg-orange-50 hover:text-primary rounded-r-xl transition-all group flex justify-between items-center" disabled={!activeStepId}>
                                        <div>
                                          <p className="font-bold text-xs">
                                            {item.name}
                                          </p>
                                          <p className="text-[10px] opacity-70 uppercase font-medium">
                                            {item.type === "recipe" ? t("erp_65") : item.category}
                                          </p>
                                        </div>
                                        <Plus className="w-3 h-3 opacity-0 group-hover:opacity-100" />
                                      </button>
                                  ...
                                  {snapshot.isDragging && <div className="p-3 bg-orange-500 text-white rounded-xl border border-orange-600 shadow-lg flex justify-between items-center">
                                      <p className="font-bold text-xs">
                                        {item.name}
                                      </p>
                                      <Plus className="w-3 h-3" />
        ...
        <footer className="p-4 md:p-8 border-t border-border bg-muted/20 flex flex-col md:flex-row justify-between items-center gap-4 md:gap-0 shrink-0 mt-auto">
          <div className="flex gap-4 md:gap-8 w-full md:w-auto justify-between md:justify-start">
            <div>
              ...
              <p className="text-xl md:text-2xl font-black text-primary">
                {steps.reduce((sum, s) => {
                const mins = s.parameters.filter((p: any) => p.unit === "min").reduce((pSum: number, p: any) => pSum + p.value, 0);
                const hrs = s.parameters.filter((p: any) => p.unit === "hr").reduce((pSum: number, p: any) => pSum + p.value, 0);
          ...
          <div className="flex gap-2 md:gap-4 w-full md:w-auto">
            {initialData?.id && <button type="button" onClick={handleDelete} disabled={loading} className="px-3 md:px-6 py-3 md:py-4 border border-destructive/20 text-destructive bg-destructive/5 hover:bg-destructive hover:text-white rounded-2xl font-bold transition-colors flex items-center gap-2 text-sm md:text-base flex-1 md:flex-none justify-center">
                <Trash2 className="w-4 h-4 md:w-5 md:h-5" />
                <span className="hidden md:inline">{t("erp_69")}</span>
                <span className="inline md:hidden">{t("erp_70")}</span>
              </button>}
            <button onClick={onClose} className="px-4 md:px-8 py-3 md:py-4 border border-border rounded-2xl font-bold text-gray-600 hover:bg-white transition-all text-sm md:text-base flex-1 md:flex-none">
              {t("erp_71")}
            </button>
            <button onClick={handleSubmit} disabled={loading || !name} className="px-6 md:px-10 py-3 md:py-4 bg-primary text-white rounded-2xl font-black shadow-xl shadow-primary/30 hover:scale-105 active:scale-95 disabled:opacity-50 transition-all flex items-center justify-center gap-2 text-sm md:text-base flex-[2] md:flex-none">
