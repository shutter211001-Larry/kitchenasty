...
export const SupplierPriceModal = ({ supplier, onClose }: Props) => {
  ...
  const handleSubmitPrice = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(editingPriceId ? '修改報價失敗' : '新增報價失敗');
    } finally {
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 flex flex-col">
          ...
          <form onSubmit={handleSubmitPrice} className="bg-muted/30 border border-border rounded-2xl p-5 space-y-4 shrink-0">
            <div className="flex items-center gap-1.5 mb-1">
              ...
              <h4 className="text-xs font-black text-gray-800 uppercase tracking-wider">
                {editingPriceId ? '編輯採購報價合約' : '登錄全新採購報價'}
              </h4>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="space-y-1 md:col-span-2 relative" onClick={(e) => e.stopPropagation()}>
                <label className="text-[10px] font-bold text-gray-600">選擇原物料食材 (輸入關鍵字搜尋)</label>
                ...
                {!selectedIngredient ? (
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      placeholder="輸入食材名稱關鍵字搜尋 (例如: 起司、莫札瑞拉)..."
                      ...
                      <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-border rounded-2xl shadow-xl max-h-52 overflow-y-auto z-50 divide-y divide-border">
                        {ingredients
                          ...
                          .map(ing => (
                            ...
                            >
                              <div>
                                <p className="text-xs font-bold text-gray-800 group-hover:text-primary transition-colors">{ing.name}</p>
                                <p className="text-[10px] text-muted-foreground mt-0.5 font-medium">
                                  {ing.category || '未分類'} · 單位: {ing.unit}
                                </p>
                              </div>
                  ...
                  <div className="flex items-center justify-between px-3 py-1.5 bg-emerald-50 border border-emerald-100 rounded-xl animate-in zoom-in-95 duration-150">
                    <div>
                      <p className="text-xs font-black text-emerald-800">{selectedIngredient.name}</p>
                      <p className="text-[10px] text-emerald-600 font-semibold mt-0.5">
                        {selectedIngredient.category || '未分類'} · 儲存單位: {selectedIngredient.unit}
                      </p>
                    </div>
                    <button
                      type="button"
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-600">包裝規格大小</label>
                <input 
                  type="number" 
                  step="any"
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-600">規格單位</label>
                <select
              ...
              <div className="space-y-1 md:col-span-2">
                <label className="text-[10px] font-bold text-gray-600">合約包裝價格 (NT$)</label>
                <input 
                  type="number" 
                  step="any"
                  className="w-full px-3 py-2 bg-white border border-border rounded-xl outline-none focus:ring-2 focus:ring-primary/20 transition-all font-mono font-bold text-xs text-right"
          ...
          <div className="flex-1 flex flex-col min-h-0">
            <div className="flex items-center gap-1.5 mb-3 shrink-0">
              ...
              <h4 className="text-xs font-black text-gray-400 uppercase tracking-wider">現有合約報價單明細</h4>
            </div>
              ...
              <div className="overflow-y-auto flex-1">
                {loading ? (
                  <div className="py-16 text-center text-muted-foreground flex flex-col items-center gap-2">
                    ...
                    <span className="text-xs font-medium">資料更新中...</span>
                  </div>
                ) : prices.length === 0 ? (
                  <div className="py-16 text-center text-muted-foreground flex flex-col items-center gap-2.5">
                    ...
                    <p className="text-xs font-bold text-gray-400">目前尚無此供應商的報價單資料</p>
                  </div>
                ) : (
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-muted/30 border-b border-border sticky top-0 z-10">
                        <th className="px-4 py-3 font-bold text-[10px] uppercase text-muted-foreground">食材名稱</th>
                        <th className="px-4 py-3 font-bold text-[10px] uppercase text-muted-foreground">分類</th>
                        <th className="px-4 py-3 font-bold text-[10px] uppercase text-muted-foreground text-right">包裝規格</th>
                        <th className="px-4 py-3 font-bold text-[10px] uppercase text-muted-foreground text-right">合約價</th>
                        <th className="px-4 py-3 font-bold text-[10px] uppercase text-muted-foreground text-right">基準單價</th>
                        <th className="px-4 py-3 font-bold text-[10px] uppercase text-muted-foreground text-center">狀態</th>
                        <th className="px-4 py-3 font-bold text-[10px] uppercase text-muted-foreground text-center">操作</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-border">
                      {prices.map(p => (
                        <tr key={p.id} className="hover:bg-muted/20 transition-colors text-xs">
                          <td className="px-4 py-3.5 font-bold text-gray-800">{p.ingredient?.name}</td>
                          <td className="px-4 py-3.5 text-muted-foreground font-semibold">
                            <span className="px-2 py-0.5 bg-muted rounded text-[10px]">
                              {p.ingredient?.category || '未分類'}
                            </span>
                          </td>
