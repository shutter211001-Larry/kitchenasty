...
export default function AllergenList() {
  const { t } = useTranslation();
    const LANGUAGES = [
      { code: 'en', label: t('autoGen.admin.key32') },
      { code: 'ja', label: t('autoGen.admin.key33') },
      { code: 'ko', label: t('autoGen.admin.key34') },
      { code: 'th', label: t('autoGen.admin.key35') },
      { code: 'tl', label: t('autoGen.admin.key36') },
      { code: 'vi', label: t('autoGen.admin.key37') },
      { code: 'id', label: t('autoGen.admin.key38') },
    ];
  ...
  async function handleDelete(id: string) {
    if (!confirm(t('autoGen.admin.key39'))) return;
    try {
  ...
  if (loading) return <div className="p-6 text-gray-500 text-center">{t('autoGen.admin.key40')}</div>;

  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">{t('autoGen.admin.key41')}</h1>
        <button
          onClick={() => setIsAdding(!isAdding)}
          className="bg-primary-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-primary-700 transition-colors"
        >
          {isAdding ? t('autoGen.admin.key42') : t('autoGen.admin.key43')}
        </button>
      </div>
        ...
        <form onSubmit={handleCreate} className="bg-white border border-gray-200 rounded-lg p-6 mb-6 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('autoGen.admin.key44')}</label>
              <input
                type="text"
                value={newName}
                onChange={(e) => setNewName(e.target.value)}
                required
                placeholder={t('autoGen.admin.key45')}
            ...
            <div className="md:col-span-2 grid grid-cols-2 sm:grid-cols-4 gap-3 mt-2 bg-gray-50 p-4 rounded-lg">
              <p className="col-span-full text-xs font-bold text-gray-400 uppercase tracking-wider">{t('autoGen.admin.key46')}</p>
              {LANGUAGES.map((lang) => (
                <div key={lang.code}>
          ...
          <div className="flex justify-end mt-4">
            ...
            >
              {isSaving ? t('autoGen.admin.key47') : t('autoGen.admin.key48')}
            </button>
          </div>
        </form>
      )}
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
        <table className="w-full text-left">
          <thead className="bg-gray-50 text-xs font-bold text-gray-500 uppercase tracking-wider">
            <tr>
              <th className="px-6 py-3">{t('autoGen.admin.key49')}</th>
              <th className="px-6 py-3">{t('autoGen.admin.key50')}</th>
              <th className="px-6 py-3">{t('autoGen.admin.key51')}</th>
              <th className="px-6 py-3 text-right">{t('autoGen.admin.key52')}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-200">
            {allergens.length === 0 && (
              <tr>
                <td colSpan={4} className="px-6 py-8 text-center text-gray-400">{t('autoGen.admin.key53')}</td>
              </tr>
            )}
            {allergens.map((a) => (
              <tr key={a.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4">
                  <span className="text-xs text-gray-500">{a._count?.menuItems || 0} {t('autoGen.admin.key54')}</span>
                </td>
                <td className="px-6 py-4 text-right">
                  <button onClick={() => handleDelete(a.id)} className="text-red-500 hover:text-red-700 text-sm">
                    {t('autoGen.admin.key55')}
                  </button>
                </td>
              </tr>
            ))}
