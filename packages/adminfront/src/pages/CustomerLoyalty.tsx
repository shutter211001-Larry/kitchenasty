...
export default function CustomerLoyalty() {
  ...
  const fetchMenuAndRules = async () => {
    ...
    try {
      ...
      setError(t('customerLoyalty.loadSettingsFailed'));
    } finally {
  ...
  const searchCustomer = async () => {
    ...
    try {
      ...
      if (res.data.length === 0) {
        setError(t('customerLoyalty.customerNotFound'));
        setCustomer(null);
        return;
      }
  ...
  const handleAdjust = async () => {
    ...
    if (isNaN(points) || points === 0) {
      setError(t('customerLoyalty.invalidPointsValue'));
      return;
    }
  ...
  const handleSaveRules = async () => {
    ...
    try {
      ...
      setSuccess(t('customerLoyalty.settingsUpdatedSuccessfully'));
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } catch (err: any) {
      setError(err.message || t('customerLoyalty.updateFailed'));
    } finally {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">{t('customerLoyalty.memberControlCenterTitle')}</h1>
          <p className="mt-1 text-sm text-gray-500">{t('customerLoyalty.memberControlCenterDesc')}</p>
        </div>
      </div>
      ...
      {error && (
        <div className="bg-red-50 border-l-4 border-red-500 text-red-700 p-4 rounded-r-xl mb-6 shadow-sm transition-all duration-300">
          <p className="font-semibold">{t('customerLoyalty.errorMessage')}</p>
      ...
      {success && (
        <div className="bg-green-50 border-l-4 border-green-500 text-green-700 p-4 rounded-r-xl mb-6 shadow-sm transition-all duration-300">
          <p className="font-semibold">{t('customerLoyalty.operationSuccess')}</p>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
              <span>{t('customerLoyalty.findCustomer')}</span>
            </h2>
            <div className="flex gap-2">
              <input
                type="email"
                placeholder={t('customerLoyalty.enterCustomerEmail')}
                value={searchEmail}
                ...
                aria-label={t('customerLoyalty.searchCustomerByEmail')}
              />
              <button
              ...
              >
                {loading ? t('customerLoyalty.searching') : t('customerLoyalty.search')}
              </button>
            </div>
          </div>
            ...
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="bg-gradient-to-br from-primary-600 to-orange-500 rounded-2xl shadow-md text-white p-6">
                <div className="flex items-start justify-between">
                  <div>
                    <span className="bg-white/20 text-xs px-2.5 py-1 rounded-full font-medium">{t('customerLoyalty.storeGoldMember')}</span>
                    <h3 className="text-xl font-bold mt-3">{customer.name}</h3>
                    <p className="text-sm text-white/80 mt-1">{customer.email}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-white/70">{t('customerLoyalty.availableRewardPoints')}</p>
                    <p className="text-4xl font-extrabold mt-1">{customer.loyaltyPoints}</p>
                    <p className="text-xs text-white/80 mt-1">{t('customerLoyalty.equivalentToNtd')} {(customer.loyaltyPoints / 100).toFixed(1)} {t('customerLoyalty.currencyValue')}</p>
                  </div>
                </div>
              </div>
              ...
              {/* Adjust Points Card */}
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                <h2 className="text-lg font-bold text-gray-900 mb-4">{t('customerLoyalty.adjustPointsTitle')}</h2>
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">{t('customerLoyalty.increaseOrDecreasePoints')}</label>
                    <input
                      type="number"
                      placeholder={t('customerLoyalty.examplePointsValue')}
                      value={adjustPoints}
                      ...
                      aria-label={t('customerLoyalty.adjustPointsAmount')}
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">{t('customerLoyalty.adjustmentReasonRemark')}</label>
                    <input
                      type="text"
                      placeholder={t('customerLoyalty.remarkDescription')}
                      value={adjustDesc}
                      onChange={(e) => setAdjustDesc(e.target.value)}
                      className="w-full px-3.5 py-2.5 border border-gray-300 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all"
                      aria-label={t('customerLoyalty.adjustmentReason')}
                    />
                  </div>
                  <button
                  ...
                  >
                    {t('customerLoyalty.confirmAdjustPoints')}
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="lg:col-span-7">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              <div>
                <h2 className="text-xl font-bold text-gray-900">{t('customerLoyalty.productRewardRedemptionPanel')}</h2>
                <p className="text-xs text-gray-400 mt-1">{t('customerLoyalty.enableProductDiscountLimit')}</p>
              </div>
              <button
              ...
              >
                {rulesSaving ? t('customerLoyalty.saving') : t('customerLoyalty.saveRewardSettings')}
              </button>
            </div>
            ...
            <div className="mb-4 relative">
              <input
                type="text"
                placeholder={t('customerLoyalty.searchProductName')}
                value={menuSearch}
            ...
            {rulesLoading ? (
              <div className="text-center py-12">
                <p className="text-gray-500">{t('customerLoyalty.loadingStoreProductsRules')}</p>
              </div>
            ) : (
              <div className="border border-gray-100 rounded-xl overflow-hidden divide-y divide-gray-100 max-h-[500px] overflow-y-auto">
                {filteredMenuItems.length === 0 ? (
                  <div className="text-center py-12 text-gray-500 text-sm">{t('customerLoyalty.noMatchingProducts')}</div>
                ) : (
                      ...
                      <div key={item.id} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-gray-50 transition-colors">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <span className="text-xs font-semibold bg-gray-100 text-gray-600 px-2 py-0.5 rounded">{t('customerLoyalty.storePriceNtd')} {item.price} {t('customerLoyalty.currencyUnit')}</span>
                            {!item.isActive && (
                              <span className="text-xs font-semibold bg-red-100 text-red-600 px-2 py-0.5 rounded">{t('customerLoyalty.offShelves')}</span>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-4 sm:gap-6">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-medium text-gray-500">{t('customerLoyalty.redeemable')}</span>
                            <button
                              type="button"
                          ...
                          <div className="flex items-center gap-2 w-32 sm:w-36">
                            <span className="text-xs font-medium text-gray-500">{t('customerLoyalty.maximumLimit')}</span>
                            <div className="relative rounded-lg shadow-sm w-full">
                              ...
                              <input
                                ...
                                placeholder={t('customerLoyalty.unlimited')}
