...
KitchenAsty 是一個包含多個模組的大型系統。在您的架構中，包含了 **2 個資料庫**、**1 個後端伺服器 (Server)** 以及 **3 個前端應用 (Frontends)**。
...
2. **新增主資料庫**：
   ...
   - 建立完成後，您可以點選卡片的 **Settings** 將其重新命名為 `KitchenAsty DB` 以防混淆。
3. **新增 ERP 資料庫**：
...
| `DATABASE_URL` | 填入第一階段取得的 `KitchenAsty DB` 連線字串 |
...
| `EMAIL_FROM` | 寄件人名稱，如 `KitchenAsty <noreply@yourdomain.com>` |
