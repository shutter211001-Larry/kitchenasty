...
|--------|-------------|-----------------|-------|
| **Full build** (EAS Build) | Native code, dependencies, app config | Yes (store review) | 1-7 days |
...
2. Add it to GitHub: Repository → Settings → Secrets → New repository secret
...
- [ ] For full build: `eas build` then `eas submit` for each platform
