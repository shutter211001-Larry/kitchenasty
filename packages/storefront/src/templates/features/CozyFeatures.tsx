...
export default function CozyFeatures({ features, t, lang = 'zh-TW' }: FeaturesProps) {const { settings } = useTheme();

  const items = features?.length ? features.filter(f => {
              if (!settings.navShowLocations && (f.title.includes(t('cozyFeatures.branch')) || f.title.includes(t('cozyFeatures.location')) || f.title.includes(t('cozyFeatures.reservation')))) return false;
              if ((!settings.navShowReservations || !settings.reservationSettings?.enabled) && f.title.includes(t('cozyFeatures.booking'))) return false;
              return true;
            }) : [
