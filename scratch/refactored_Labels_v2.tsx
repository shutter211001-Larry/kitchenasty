...
export const Labels = () => {
  ...
  const fetchManufacturers = async () => {
    try {
      const response = await axios.get("http://localhost:3000/api/settings/manufacturers");
  ...
  const saveManufacturers = async (newManufacturers: ManufacturerProfile[]) => {
    try {
      await axios.put("http://localhost:3000/api/settings/manufacturers", { manufacturers: newManufacturers });
  ...
  const updateNutritionConfig = (key: string, updates: Partial<{
    decimals: number;
    maxLength: number;
  }>) => {
  ...
  const handleUpdateLine = (id: string, updates: Partial<CustomLine>) => {
    setCustomLines(prev => prev.map(l => l.id === id ? {
      ...l,
      ...updates
    } : l));
  };
      ...
      setCustomLines(prevLines => {
        ...
        return prevLines.map(line => {
          ...
          if (draggingLineId) {
            ...
            let newLength = line.type === "horizontal" ? Math.round((start.length + deltaXPercent) / 2) * 2 : Math.round((start.length + deltaYPercent) / 2) * 2;
  ...
  const applyRecipeData = (recipe: any, scale: number) => {
    ...
    if (recipe.totalIngredients) {
      recipe.totalIngredients.forEach((ing: any) => {
        ...
        const name = ing.name;
        if (name.includes(i18n.t("erp_476")) || name.includes(i18n.t("erp_477")) || name.includes(i18n.t("erp_478"))) allergensSet.add(i18n.t("erp_479"));
        if (name.includes(i18n.t("erp_480")) || name.includes(i18n.t("erp_481")) || name.includes(i18n.t("erp_482")) || name.includes(i18n.t("erp_483"))) allergensSet.add(i18n.t("erp_484"));
        if (name.includes(i18n.t("erp_485"))) allergensSet.add(i18n.t("erp_486"));
        if (name.includes(i18n.t("erp_487")) || name.includes(i18n.t("erp_488"))) allergensSet.add(i18n.t("erp_489"));
        if (name.includes(i18n.t("erp_490")) || name.includes(i18n.t("erp_491")) || name.includes(i18n.t("erp_492")) || name.includes(i18n.t("erp_493"))) allergensSet.add(i18n.t("erp_494"));
        if (name.includes(i18n.t("erp_495")) || name.includes(i18n.t("erp_496")) || name.includes(i18n.t("erp_497"))) allergensSet.add(i18n.t("erp_498"));
        if (name.includes(i18n.t("erp_499"))) allergensSet.add(i18n.t("erp_499"));
        if (name.includes(i18n.t("erp_500")) || name.includes(i18n.t("erp_501"))) allergensSet.add(i18n.t("erp_502"));
      });
    }
...
const handleSaveSettings = () => {
  ...
  const renderLabelLogo = () => {
    if (logoType === "upload" && uploadedLogo) {
      return <img src={uploadedLogo} alt="Custom Logo" style={{
        width: "100%",
        height: "100%",
        objectFit: "contain"
    ...
    if (logoType === "text") {
      const abbreviation = brandNameZh ? brandNameZh.substring(0, 2) : t("erp_505");
      return <div className="w-full h-full bg-black text-white flex items-center justify-center font-black text-[9pt] leading-none select-none">
          {abbreviation}
        </div>;
    }

    // Default to Vector Icon
    const IconComponent = selectedIconName === "ChefHat" ? ChefHat : selectedIconName === "Utensils" ? Utensils : selectedIconName === "Flame" ? Flame : selectedIconName === "Award" ? Award : selectedIconName === "Sparkles" ? Sparkles : selectedIconName === "Layers" ? Layers : ChefHat;
  ...
  return <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pb-4 border-b border-border print:hidden">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          <button onClick={handleSaveSettings} className="flex-1 md:flex-none px-4 py-2 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-xl font-bold text-xs shadow-sm hover:bg-emerald-100 transition-all active:scale-95 flex items-center justify-center gap-1.5 cursor-pointer">
            <Save className="w-3.5 h-3.5" />
            <span>{t("erp_508")}</span>
          </button>

          <button onClick={handleReset} className="flex-1 md:flex-none px-4 py-2 bg-white border border-border text-gray-700 rounded-xl font-bold text-xs shadow-sm hover:bg-muted transition-all active:scale-95 flex items-center justify-center gap-1.5 cursor-pointer">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 bg-white border border-border rounded-3xl p-6 space-y-6 shadow-sm print:hidden max-h-[82vh] overflow-y-auto">
          ...
          <div className="bg-primary/5 border border-primary/10 rounded-2xl p-4 space-y-3">
            <div className="flex items-center justify-between text-primary">
              ...
              {loading && <div className="w-3.5 h-3.5 border-2 border-primary border-t-transparent rounded-full animate-spin shrink-0"></div>}
            </div>
            <p className="text-[10px] text-muted-foreground font-semibold">
              {t("erp_512")}
            </p>
            <select disabled={loading} className="w-full px-3 py-2 bg-white border border-border rounded-xl outline-none focus:ring-2 focus:ring-primary/20 transition-all font-bold text-xs disabled:opacity-60 disabled:bg-muted" value={selectedRecipeId} onChange={e => handleRecipeChange(e.target.value)}>
              <option value="">{t("erp_513")}</option>
              {recipes.filter(recipe => recipe.isProduct !== false).map(recipe => <option key={recipe.id} value={recipe.id}>
                    🍳 {recipe.name}
                    {t("erp_514")}
                    {recipe.yieldAmount}
                    {recipe.yieldUnit})
            ...
            {selectedRecipeId && <div className="space-y-1.5 animate-in fade-in slide-in-from-top-1 duration-150 mt-2 border-t border-primary/10 pt-2">
                ...
                <input type="number" step="0.1" min="0.1" max="10" className="w-full px-3 py-1.5 bg-white border border-border rounded-xl font-bold text-xs outline-none focus:ring-2 focus:ring-primary/20" value={portionScale} onChange={e => handlePortionScaleChange(parseFloat(e.target.value) || 1.0)} />
                <span className="text-[8.5px] text-muted-foreground leading-normal block pb-1.5">
                  {t("erp_517")}
                </span>

                {loadedRecipe && loadedRecipe.bakingLossRate > 0 && <div className="bg-orange-50 border border-orange-200 rounded-2xl p-3.5 my-2.5 text-[10px] text-orange-800 space-y-1 animate-in fade-in slide-in-from-top-1 duration-200">
                    ...
                    <p className="leading-relaxed font-medium">
                      {t("erp_520")}
                      {(loadedRecipe.totalWeight / (1 - loadedRecipe.bakingLossRate / 100) * portionScale).toFixed(0)}
                      {t("erp_521")}
                ...
                <button type="button" onClick={handleSaveLabelConfig} disabled={savingLabelConfig} className="w-full py-2.5 bg-primary hover:bg-primary/90 text-white rounded-xl font-black text-xs shadow-md active:scale-95 transition-all flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50">
                  <span>{t("erp_523")}</span>
                </button>
                ...
                <div className="bg-slate-50 border border-slate-200 p-3.5 rounded-2xl space-y-3 mt-3.5 animate-in fade-in duration-200">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button type="button" onClick={handleCopyLayoutToClipboard} className="py-1.5 px-2 border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 rounded-xl font-black text-[10px] flex items-center justify-center gap-1 cursor-pointer transition-colors active:scale-95">
                      {t("erp_525")}
                    </button>
                    <button type="button" onClick={handlePasteLayoutFromClipboard} className="py-1.5 px-2 border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 rounded-xl font-black text-[10px] flex items-center justify-center gap-1 cursor-pointer transition-colors active:scale-95">
                      {t("erp_526")}
                    </button>
                  </div>
                  ...
                  <div className="space-y-1.5 pt-1.5 border-t border-dashed border-slate-200">
                    ...
                    <div className="flex gap-1.5">
                      <select className="flex-1 px-2.5 py-1.5 bg-white border border-border rounded-xl font-bold text-[11px] outline-none" value={targetRecipeIdToCopyTo} onChange={e => setTargetRecipeIdToCopyTo(e.target.value)}>
                        <option value="">{t("erp_528")}</option>
                        {recipes.filter(r => r.id !== selectedRecipeId).map(r => <option key={r.id} value={r.id}>
                              {r.name}
                            </option>)}
                      </select>
                      <button type="button" onClick={handleCopyLayoutToRecipe} disabled={copyingLayoutToRecipe || !targetRecipeIdToCopyTo} className="py-1.5 px-3 bg-teal-600 hover:bg-teal-700 disabled:bg-slate-200 disabled:text-slate-400 text-white rounded-xl font-black text-[10px] flex items-center justify-center gap-1 cursor-pointer transition-colors disabled:cursor-not-allowed">
          ...
          <div className="space-y-3 bg-indigo-50/50 border border-indigo-100 p-4 rounded-2xl">
            ...
            <div className="flex items-center gap-3">
              <label className="flex items-center gap-2 cursor-pointer text-xs font-bold text-gray-700">
                <input type="checkbox" className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 w-4 h-4 cursor-pointer" checked={enableDualLanguage} onChange={e => setEnableDualLanguage(e.target.checked)} />
                {t("erp_532")}
              </label>

              {enableDualLanguage && <select className="flex-1 px-3 py-1.5 bg-white border border-border rounded-xl font-bold text-xs outline-none" value={printLanguage} onChange={e => setPrintLanguage(e.target.value)}>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              label: "70x50 mm",
              desc: t("erp_538")
            }].map(opt => <button key={opt.id} type="button" onClick={() => setLabelSize(opt.id as LabelSize)} className={cn("p-3 border rounded-xl text-left transition-all active:scale-98", labelSize === opt.id ? "border-primary bg-primary/5 text-primary shadow-sm ring-1 ring-primary/20" : "border-border hover:bg-muted text-gray-700")}>
          ...
          <div className="space-y-3 bg-slate-50 border border-slate-200/60 p-4 rounded-2xl animate-in fade-in duration-200">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3">
                <input type="range" min="0" max="10" step="0.5" className="flex-1 h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-primary" value={labelGap} onChange={e => setLabelGap(parseFloat(e.target.value))} />
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 pt-1">
                ...
                value: 4,
                label: "4mm"
              }].map(pill => <button key={pill.value} type="button" onClick={() => setLabelGap(pill.value)} className={cn("px-2 py-1 text-[9.5px] font-black rounded-lg border transition-all cursor-pointer", labelGap === pill.value ? "bg-slate-900 border-slate-900 text-white shadow-sm" : "bg-white border-border text-gray-600 hover:bg-slate-50")}>
                    {pill.label}
                  </button>)}
              </div>
            </div>
            ...
            <div className="space-y-2 pt-2.5 border-t border-slate-200">
              ...
              <div className="flex items-center gap-3">
                <input type="range" min="0" max="10" step="0.5" className="flex-1 h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-primary" value={labelBorderRadius} onChange={e => setLabelBorderRadius(parseFloat(e.target.value))} />
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 pt-1">
                ...
                value: 6,
                label: "6mm"
              }].map(pill => <button key={pill.value} type="button" onClick={() => setLabelBorderRadius(pill.value)} className={cn("px-2 py-1 text-[9.5px] font-black rounded-lg border transition-all cursor-pointer", labelBorderRadius === pill.value ? "bg-slate-900 border-slate-900 text-white shadow-sm" : "bg-white border-border text-gray-600 hover:bg-slate-50")}>
                    {pill.label}
                  </button>)}
              </div>
            </div>
            ...
            <div className="space-y-2 pt-2.5 border-t border-slate-200">
              <div className="flex items-center justify-between">
                ...
                <span className="px-1.5 py-0.5 bg-slate-100 text-slate-700 text-[9px] font-black rounded font-mono uppercase">
                  {gapAlignment === "top" ? t("erp_548") : gapAlignment === "center" ? t("erp_549") : t("erp_550")}
                </span>
              </div>
              ...
              <div className="grid grid-cols-3 gap-1 bg-white border border-border p-1 rounded-xl">
                ...
                label: t("erp_556"),
                desc: t("erp_557")
              }].map(t => <button key={t.id} type="button" onClick={() => setGapAlignment(t.id as any)} className={cn("py-1 px-1 text-[9.5px] font-black rounded-lg transition-all cursor-pointer flex flex-col items-center justify-center gap-0.5", gapAlignment === t.id ? "bg-slate-900 text-white shadow-sm" : "text-slate-500 hover:bg-slate-50 hover:text-slate-700")}>
                    <span>{t.label}</span>
            ...
            <div className="space-y-2.5 pt-2 border-t border-slate-200">
              ...
              <label className="flex items-start gap-2.5 cursor-pointer select-none">
                <input type="checkbox" checked={includeGapInPrint} onChange={e => setIncludeGapInPrint(e.target.checked)} className="w-3.5 h-3.5 accent-primary rounded border-border focus:ring-primary/20 shrink-0 mt-0.5" />
              ...
              <label className="flex items-start gap-2.5 cursor-pointer select-none">
                <input type="checkbox" checked={previewContinuous} onChange={e => setPreviewContinuous(e.target.checked)} className="w-3.5 h-3.5 accent-primary rounded border-border focus:ring-primary/20 shrink-0 mt-0.5" />
                <div className="flex-1 min-w-0">
                  <span className="text-[10.5px] font-bold text-gray-700 block leading-tight">
                    {t("erp_560")}
                  </span>
          ...
          <div className="bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-100/80 rounded-2xl p-4 space-y-3 shadow-sm text-black">
            <div className="flex items-center justify-between">
              ...
              <label className="relative inline-flex items-center cursor-pointer select-none">
                <input type="checkbox" checked={useCustomLayout} onChange={e => setUseCustomLayout(e.target.checked)} className="sr-only peer" />
                <div className="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600"></div>
              </label>
            </div>
            {useCustomLayout && <div className="text-[9px] text-indigo-800 leading-relaxed font-semibold bg-white/70 rounded-xl p-2.5 border border-indigo-100 flex flex-col gap-1">
                <span>{t("erp_564")}</span>
                <span>
                  {t("erp_565")}
          ...
          <div className="space-y-3">
            ...
            <div className={cn("border rounded-2xl overflow-hidden bg-white shadow-sm transition-all", activeAccordion === "A" ? "border-primary/40 ring-1 ring-primary/10" : "border-border")}>
              <button type="button" onClick={() => setActiveAccordion(activeAccordion === "A" ? null : "A")} className="w-full px-4 py-3 flex items-center justify-between bg-slate-50 hover:bg-slate-100/80 transition-colors cursor-pointer">
                <div className="flex items-center gap-2">
              ...
              {activeAccordion === "A" && <div className="p-4 border-t border-border space-y-4 animate-in fade-in duration-200">
                  {/* Font Scale Slider */}
                  <div className="bg-slate-50 border border-slate-200/60 p-3 rounded-2xl space-y-1.5 shrink-0">
                    ...
                    <input type="range" min="0.6" max="2.0" step="0.05" value={groupFontScales.A} onChange={e => updateGroupFontScale("A", parseFloat(e.target.value))} className="w-full h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-orange-600" />
                  </div>
                  ...
                  <div className="flex items-center justify-between pb-1.5 border-b border-dashed border-slate-100">
                    <label className="flex items-center gap-2 cursor-pointer font-black text-[10.5px] text-gray-700">
                      <input type="checkbox" checked={showReheating} onChange={e => setShowReheating(e.target.checked)} className="w-3.5 h-3.5 accent-primary rounded border-border" />
                      <span>{t("erp_578")}</span>
                    </label>
                  </div>

                  {showReheating && <div className="space-y-3.5">
                      <div className="space-y-1">
                        <label className="text-[9.5px] font-bold text-gray-600">
                          {t("erp_579")}
                        </label>
                        <input type="text" className="w-full px-3 py-1.5 bg-slate-50 border border-border rounded-xl font-bold text-xs outline-none focus:ring-2 focus:ring-primary/10" value={reheatingMainTitle} onChange={e => setReheatingMainTitle(e.target.value)} />
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <div className="space-y-1">
                          ...
                          <input type="text" className="w-full px-2 py-1.5 bg-slate-50 border border-border rounded-xl font-bold text-xs outline-none focus:ring-2 focus:ring-primary/10" value={airFryerTitle} onChange={e => setAirFryerTitle(e.target.value)} />
                        </div>
                        <div className="space-y-1">
                          <label className="text-[8.5px] font-bold text-gray-600">
                            {t("erp_581")}
                          </label>
                          <input type="text" className="w-full px-2 py-1.5 bg-slate-50 border border-border rounded-xl font-bold text-xs outline-none focus:ring-2 focus:ring-primary/10" value={ovenTitle} onChange={e => setOvenTitle(e.target.value)} />
                        </div>
                        <div className="space-y-1">
                          <label className="text-[8.5px] font-bold text-gray-600">
                            {t("erp_582")}
                          </label>
                          <input type="text" className="w-full px-2 py-1.5 bg-slate-50 border border-border rounded-xl font-bold text-xs outline-none focus:ring-2 focus:ring-primary/10" value={panTitle} onChange={e => setPanTitle(e.target.value)} />
                        </div>
                      </div>
                    ...
                    }].map((sub, idx) => <label key={idx} className="flex items-center gap-1.5 cursor-pointer select-none">
                              <input type="checkbox" checked={sub.state} onChange={e => sub.setter(e.target.checked)} className="w-3.5 h-3.5 accent-primary rounded border-border focus:ring-primary/20 cursor-pointer" />
                      ...
                      {showAirFryer && <div className="space-y-1 animate-in fade-in duration-150">
                          <label className="text-[9px] font-bold text-gray-500">
                            {airFryerTitle}
                            {t("erp_584")}
                          </label>
                          <textarea rows={2} className="w-full px-3 py-1.5 bg-slate-50 border border-border rounded-xl font-bold text-xs" value={airFryerSteps} onChange={e => setAirFryerSteps(e.target.value)} />
                        </div>}

                      {showOven && <div className="space-y-1 animate-in fade-in duration-150">
                          <label className="text-[9px] font-bold text-gray-500">
                            {ovenTitle}
                            {t("erp_584")}
                          </label>
                          <textarea rows={2} className="w-full px-3 py-1.5 bg-slate-50 border border-border rounded-xl font-bold text-xs" value={ovenSteps} onChange={e => setOvenSteps(e.target.value)} />
                        </div>}

                      {showPan && <div className="space-y-1 animate-in fade-in duration-150">
                          <label className="text-[9px] font-bold text-gray-500">
                            {panTitle}
                            {t("erp_584")}
                          </label>
                          <textarea rows={2} className="w-full px-3 py-1.5 bg-slate-50 border border-border rounded-xl font-bold text-xs" value={panSteps} onChange={e => setPanSteps(e.target.value)} />
                        </div>}
                      ...
                      <div className="bg-slate-50 border border-slate-100 p-3 rounded-2xl space-y-2 mt-2">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          <div className="space-y-1">
                            ...
                            <input type="range" min="5" max="12" step="0.1" value={reheatingMainTitleSize} onChange={e => setReheatingMainTitleSize(Number(e.target.value))} className="w-full h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-primary" />
                          </div>
                          <div className="space-y-1">
                            <label className="text-[8px] font-bold text-gray-500 block">
                              {t("erp_587")}
                            ...
                            <input type="range" min="5" max="11" step="0.1" value={reheatingSubTitleSize} onChange={e => setReheatingSubTitleSize(Number(e.target.value))} className="w-full h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-primary" />
                          </div>
                          <div className="space-y-1">
                            <label className="text-[8px] font-bold text-gray-500 block">
                              {t("erp_588")}
                            ...
                            <input type="range" min="4.5" max="10" step="0.1" value={reheatingContentSize} onChange={e => setReheatingContentSize(Number(e.target.value))} className="w-full h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-primary" />
                          </div>
                        </div>
                      </div>
                    </div>}
            ...
            <div className={cn("border rounded-2xl overflow-hidden bg-white shadow-sm transition-all", activeAccordion === "B" ? "border-primary/40 ring-1 ring-primary/10" : "border-border")}>
              <button type="button" onClick={() => setActiveAccordion(activeAccordion === "B" ? null : "B")} className="w-full px-4 py-3 flex items-center justify-between bg-slate-50 hover:bg-slate-100/80 transition-colors cursor-pointer">
                <div className="flex items-center gap-2">
              ...
              {activeAccordion === "B" && <div className="p-4 border-t border-border space-y-4 animate-in fade-in duration-200">
                  {/* Font Scale Slider */}
                  <div className="bg-slate-50 border border-slate-200/60 p-3 rounded-2xl space-y-1.5 shrink-0">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-black text-gray-700 flex items-center gap-1">
                    ...
                    <input type="range" min="0.6" max="2.0" step="0.05" value={groupFontScales.B} onChange={e => updateGroupFontScale("B", parseFloat(e.target.value))} className="w-full h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-teal-600" />
                  </div>
                  ...
                  <div className="bg-slate-50/50 p-3 border border-border rounded-2xl space-y-3">
                    <div className="flex items-center justify-between">
                      <label className="flex items-center gap-1.5 cursor-pointer font-black text-[10.5px] text-gray-700">
                        <input type="checkbox" checked={showBranding} onChange={e => setShowBranding(e.target.checked)} className="w-3.5 h-3.5 accent-primary rounded border-border" />
                        <span>{t("erp_591")}</span>
                      </label>
                    </div>

                    {showBranding && <div className="space-y-3.5 animate-in fade-in duration-150">
                        <div className="grid grid-cols-2 gap-3">
                          <div className="space-y-1">
                            <label className="text-[9.5px] font-bold text-gray-600">
                              {t("erp_592")}
                            </label>
                            <input type="text" className="w-full px-3 py-1.5 bg-white border border-border rounded-xl font-bold text-xs outline-none focus:ring-2 focus:ring-primary/20" value={brandNameZh} onChange={e => setBrandNameZh(e.target.value)} />
                          </div>
                          <div className="space-y-1">
                            <label className="text-[9.5px] font-bold text-gray-600">
                              {t("erp_593")}
                            </label>
                            <input type="text" className="w-full px-3 py-1.5 bg-white border border-border rounded-xl font-bold text-xs outline-none focus:ring-2 focus:ring-primary/20" value={brandNameEn} onChange={e => setBrandNameEn(e.target.value)} />
                          </div>
                        </div>
                        ...
                        id: "text",
                        label: t("erp_597")
                      }].map(t => <button key={t.id} type="button" onClick={() => setLogoType(t.id as any)} className={cn("py-1 text-[9.5px] font-black rounded-lg transition-all cursor-pointer", logoType === t.id ? "bg-primary text-white shadow-sm" : "text-slate-500 hover:bg-slate-50 hover:text-slate-700")}>
                                {t.label}
                              </button>)}
                          </div>
                        </div>
                        ...
                        {logoType === "icon" && <div className="space-y-1.5 pt-1 animate-in fade-in duration-200">
                        ...
                        return <button key={item.name} type="button" onClick={() => setSelectedIconName(item.name)} className={cn("flex items-center gap-1.5 px-2 py-1.5 rounded-lg border text-[9.5px] font-bold transition-all cursor-pointer justify-center", selectedIconName === item.name ? "border-primary bg-primary/5 text-primary shadow-sm" : "border-border bg-white text-gray-500 hover:bg-slate-50")}>
                        ...
                        {logoType === "upload" && <div className="space-y-1.5 pt-1 animate-in fade-in duration-200">
                          ...
                          if (file) {
                            ...
                            reader.readAsDataURL(file);
                              ...
                              <div className="flex-1 min-w-0">
                                {uploadedLogo ? <div className="flex items-center gap-2">
                                    ...
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <button type="button" onClick={() => setUploadedLogo("")} className="text-[9px] text-red-500 font-bold hover:underline cursor-pointer">
                                        {t("erp_608")}
                                      </button>
                                    </div>
                                  </div> : <span className="text-[9px] text-gray-400 font-semibold block leading-tight font-sans">
                                    {t("erp_609")}
                                  </span>}
                              </div>
                            </div>
                        ...
                        {logoType === "text" && <div className="bg-white p-2.5 border border-border rounded-xl text-[9px] text-gray-500 leading-normal font-semibold animate-in fade-in duration-200">
                            {t("erp_610")}
                            {brandNameZh ? brandNameZh.substring(0, 2) : t("erp_505")}
                            {t("erp_611")}
                          </div>}
                      </div>}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-1">
                    <div className="space-y-1">
                      ...
                      <input type="text" className="w-full px-3 py-1.5 bg-slate-50 border border-border rounded-xl font-bold text-xs" value={productZh} onChange={e => setProductZh(e.target.value)} />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-gray-600">
                        {t("erp_613")}
                      </label>
                      <input type="text" className="w-full px-3 py-1.5 bg-slate-50 border border-border rounded-xl font-bold text-xs" value={productEn} onChange={e => setProductEn(e.target.value)} />
                    </div>
                  </div>
                </div>}
            </div>
            ...
            <div className={cn("border rounded-2xl overflow-hidden bg-white shadow-sm transition-all", activeAccordion === "F" ? "border-primary/40 ring-1 ring-primary/10" : "border-border")}>
              <button type="button" onClick={() => setActiveAccordion(activeAccordion === "F" ? null : "F")} className="w-full px-4 py-3 flex items-center justify-between bg-slate-50 hover:bg-slate-100/80 transition-colors cursor-pointer">
                <div className="flex items-center gap-2">
              ...
              {activeAccordion === "F" && <div className="p-4 border-t border-border space-y-4 animate-in fade-in duration-200">
                  {/* Font Scale Slider */}
                  <div className="bg-slate-50 border border-slate-200/60 p-3 rounded-2xl space-y-1.5 shrink-0">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-black text-gray-700 flex items-center gap-1">
                    ...
                    <input type="range" min="0.6" max="2.0" step="0.05" value={groupFontScales.F} onChange={e => updateGroupFontScale("F", parseFloat(e.target.value))} className="w-full h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-amber-600" />
                  </div>
                  {/* Ingredients List */}
                  <div className="space-y-1.5">
                    <label className="flex items-center gap-2 cursor-pointer font-black text-[10.5px] text-gray-700">
                      <input type="checkbox" checked={showIngredients} onChange={e => setShowIngredients(e.target.checked)} className="w-3.5 h-3.5 accent-primary rounded border-border" />
                      <span>{t("erp_616")}</span>
                    </label>
                    {showIngredients && <div className="space-y-1">
                        <textarea rows={3} className="w-full px-3 py-1.5 bg-slate-50 border border-border rounded-xl font-bold text-xs leading-relaxed" value={ingredientsText} onChange={e => setIngredientsText(e.target.value)} placeholder={t("erp_617")} />
                    ...
                    {showIngredients && parseIngredientReferences(ingredientsText).length > 0 && <div className="mt-3 space-y-2 bg-slate-50 p-3 border border-border rounded-xl animate-in fade-in duration-150 text-left">
                    ...
                    const comps = getIngredientComponents(ref.ingredientName);
                    const isExpanded = !!expandedIngredients[ref.raw];
                    return <label key={idx} className="flex items-center gap-2 cursor-pointer select-none">
                                  <input type="checkbox" checked={isExpanded} disabled={!comps} onChange={e => {
                        setExpandedIngredients(prev => ({
                          ...
                          [ref.raw]: e.target.checked
                                  ...
                                  <span className={cn("text-[10.5px] font-bold text-gray-700", !comps && "text-gray-400")}>
                                    {t("erp_629")}
                                    {ref.customName ? `${ref.ingredientName} (${ref.customName})` : ref.ingredientName}
                  ...
                  <div className="bg-amber-50/60 p-3 border border-amber-100 rounded-2xl space-y-2">
                    <label className="flex items-center gap-2 cursor-pointer font-black text-[10.5px] text-amber-900">
                      <input type="checkbox" checked={showNotReadyToEat} onChange={e => setShowNotReadyToEat(e.target.checked)} className="w-3.5 h-3.5 accent-amber-600 rounded border-amber-200" />
                      <span>{t("erp_631")}</span>
                    </label>
                    {showNotReadyToEat && <input type="text" className="w-full px-3 py-1.5 bg-white border border-amber-200/80 rounded-xl font-bold text-xs text-amber-800 focus:ring-2 focus:ring-amber-500/10 outline-none" value={notReadyToEatText} onChange={e => setNotReadyToEatText(e.target.value)} />}
                  </div>

                  {/* Allergen Warning */}
                  <div className="space-y-1.5">
                    <label className="flex items-center gap-2 cursor-pointer font-black text-[10.5px] text-rose-700">
                      <input type="checkbox" checked={showAllergens} onChange={e => setShowAllergens(e.target.checked)} className="w-3.5 h-3.5 accent-rose-600 rounded border-rose-200" />
                      <span>{t("erp_632")}</span>
                    </label>
                    {showAllergens && <textarea rows={2} className="w-full px-3 py-1.5 bg-rose-50/40 border border-rose-100 rounded-xl font-bold text-xs text-rose-700" value={allergenWarning} onChange={e => setAllergenWarning(e.target.value)} />}
                  </div>
                </div>}
            </div>
            ...
            <div className={cn("border rounded-2xl overflow-hidden bg-white shadow-sm transition-all", activeAccordion === "C" ? "border-primary/40 ring-1 ring-primary/10" : "border-border")}>
              <button type="button" onClick={() => setActiveAccordion(activeAccordion === "C" ? null : "C")} className="w-full px-4 py-3 flex items-center justify-between bg-slate-50 hover:bg-slate-100/80 transition-colors cursor-pointer">
                <div className="flex items-center gap-2">
              ...
              {activeAccordion === "C" && <div className="p-4 border-t border-border space-y-4 animate-in fade-in duration-200">
                  {/* Font Scale Slider */}
                  <div className="bg-slate-50 border border-slate-200/60 p-3 rounded-2xl space-y-1.5 shrink-0">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-black text-gray-700 flex items-center gap-1">
                    ...
                    <input type="range" min="0.6" max="2.0" step="0.05" value={groupFontScales.C} onChange={e => updateGroupFontScale("C", parseFloat(e.target.value))} className="w-full h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-emerald-600" />
                  </div>
                  {/* Net Weight & Origin */}
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      <label className="flex items-center gap-1.5 cursor-pointer font-black text-[10px] text-gray-700">
                        <input type="checkbox" checked={showNetWeight} onChange={e => setShowNetWeight(e.target.checked)} className="w-3.5 h-3.5 accent-primary rounded border-border" />
                        <span>{t("erp_635")}</span>
                      </label>
                      {showNetWeight && <input type="text" className="w-full px-3 py-1.5 bg-slate-50 border border-border rounded-xl font-bold text-xs" value={netWeight} onChange={e => setNetWeight(e.target.value)} />}
                    </div>

                    <div className="space-y-1.5">
                      <label className="flex items-center gap-1.5 cursor-pointer font-black text-[10px] text-gray-700">
                        <input type="checkbox" checked={showOrigin} onChange={e => setShowOrigin(e.target.checked)} className="w-3.5 h-3.5 accent-primary rounded border-border" />
                        <span>{t("erp_636")}</span>
                      </label>
                      {showOrigin && <input type="text" className="w-full px-3 py-1.5 bg-slate-50 border border-border rounded-xl font-bold text-xs" value={originCountry} onChange={e => setOriginCountry(e.target.value)} />}
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-4 space-y-4">
                    ...
                    <div className="space-y-1.5">
                      <label className="flex items-center gap-1.5 cursor-pointer font-black text-[9.5px] text-gray-600">
                        <input type="checkbox" checked={showStorage} onChange={e => setShowStorage(e.target.checked)} className="w-3.5 h-3.5 accent-primary rounded border-border" />
                        <span>{t("erp_638")}</span>
                      </label>
                      {showStorage && <input type="text" className="w-full px-3 py-1.5 bg-white border border-border rounded-xl font-bold text-xs" value={storageCondition} onChange={e => setStorageCondition(e.target.value)} />}
                    </div>

                    {/* Expiry Date Toggles and Inputs */}
                    <div className="space-y-2.5 pt-1">
                      <label className="flex items-center gap-1.5 cursor-pointer font-black text-[9.5px] text-gray-600">
                        <input type="checkbox" checked={showExpiry} onChange={e => setShowExpiry(e.target.checked)} className="w-3.5 h-3.5 accent-primary rounded border-border" />
                        <span>{t("erp_639")}</span>
                      </label>

                      {showExpiry && <div className="space-y-3.5 p-3 bg-white border border-slate-100 rounded-xl animate-in fade-in duration-150">
                          <div className="flex items-center justify-between">
                            <span className="text-[9.5px] font-bold text-gray-500">
                              {t("erp_640")}
                            </span>
                            <div className="flex gap-1.5">
                              <button type="button" onClick={() => setExpiryOption("printed")} className={cn("px-2 py-0.5 rounded text-[8.5px] font-black tracking-wide cursor-pointer", expiryOption === "printed" ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-500 hover:bg-slate-200")}>
                                {t("erp_641")}
                              </button>
                              <button type="button" onClick={() => setExpiryOption("date")} className={cn("px-2 py-0.5 rounded text-[8.5px] font-black tracking-wide cursor-pointer", expiryOption === "date" ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-500 hover:bg-slate-200")}>
                                {t("erp_642")}
                              </button>
                            </div>
                          </div>

                          {expiryOption === "date" ? <div className="space-y-1 animate-in fade-in duration-150">
                              <label className="text-[8.5px] font-bold text-gray-500">
                                {t("erp_643")}
                              </label>
                              <input type="text" className="w-full px-2.5 py-1 bg-slate-50 border border-border rounded-lg font-mono font-bold text-xs" value={expiryDate} onChange={e => setExpiryDate(e.target.value)} />
                            </div> : <p className="text-[8.5px] text-muted-foreground font-semibold leading-relaxed">
                              {t("erp_644")}
                            </p>}
                          ...
                          <div className="space-y-1 pt-1 border-t border-slate-100">
                            ...
                            <input type="text" className="w-full px-2.5 py-1 bg-slate-50 border border-border rounded-lg font-bold text-xs" value={shelfLife} onChange={e => setShelfLife(e.target.value)} />
                          </div>
                        </div>}
                    </div>
                  </div>
            ...
            <div className={cn("border rounded-2xl overflow-hidden bg-white shadow-sm transition-all", activeAccordion === "D" ? "border-primary/40 ring-1 ring-primary/10" : "border-border")}>
              <button type="button" onClick={() => setActiveAccordion(activeAccordion === "D" ? null : "D")} className="w-full px-4 py-3 flex items-center justify-between bg-slate-50 hover:bg-slate-100/80 transition-colors cursor-pointer">
                <div className="flex items-center gap-2">
              ...
              {activeAccordion === "D" && <div className="p-4 border-t border-border space-y-4 animate-in fade-in duration-200">
                  {/* Font Scale Slider */}
                  <div className="bg-slate-50 border border-slate-200/60 p-3 rounded-2xl space-y-1.5 shrink-0">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-black text-gray-700 flex items-center gap-1">
                    ...
                    <input type="range" min="0.6" max="2.0" step="0.05" value={groupFontScales.D} onChange={e => updateGroupFontScale("D", parseFloat(e.target.value))} className="w-full h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-sky-600" />
                  </div>
                  <div className="flex items-center justify-between pb-1.5 border-b border-dashed border-slate-100">
                    <label className="flex items-center gap-2 cursor-pointer font-black text-[10.5px] text-gray-700">
                      <input type="checkbox" checked={showResponsible} onChange={e => setShowResponsible(e.target.checked)} className="w-3.5 h-3.5 accent-primary rounded border-border" />
                      <span>{t("erp_648")}</span>
                    </label>
                  </div>

                  {showResponsible && <div className="space-y-3.5 animate-in fade-in duration-150">
                      <div className="space-y-1">
                        <label className="text-[9.5px] font-bold text-gray-600">
                          {t("erp_649")}
                        </label>
                        <input type="text" className="w-full px-3 py-1.5 bg-slate-50 border border-border rounded-xl font-bold text-xs" value={companyName} onChange={e => setCompanyName(e.target.value)} />
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1">
                          ...
                          <input type="text" className="w-full px-3 py-1.5 bg-slate-50 border border-border rounded-xl font-bold text-xs" value={companyPhone} onChange={e => setCompanyPhone(e.target.value)} />
                        </div>
                      </div>
                      ...
                      <div className="space-y-1">
                        ...
                        <input type="text" className="w-full px-3 py-1.5 bg-slate-50 border border-border rounded-xl font-bold text-xs" value={companyAddress} onChange={e => setCompanyAddress(e.target.value)} />
                      </div>
                  ...
                  <div className="border-t border-slate-100 pt-3 space-y-2">
                    <label className="flex items-center gap-2 cursor-pointer font-bold text-[10px] text-gray-700">
                      <input type="checkbox" checked={showBarcode} onChange={e => setShowBarcode(e.target.checked)} className="w-3.5 h-3.5 accent-primary rounded border-border" />
                      <span>{t("erp_656")}</span>
                    </label>
                    {showBarcode && <div className="space-y-3 animate-in fade-in duration-150">
                        <div className="space-y-1">
                          <span className="text-[8.5px] font-bold text-gray-500 block">
                            {t("erp_657")}
                          </span>
                          <input type="text" className="w-full px-3 py-1.5 bg-slate-50 border border-border rounded-xl font-bold text-xs font-mono" value={barcodeText} onChange={e => setBarcodeText(e.target.value)} />
                        </div>
                        <div className="space-y-1">
                          <span className="text-[8.5px] font-bold text-gray-500 block">
                            {t("erp_658")}
                          </span>
                          <textarea className="w-full px-3 py-1.5 bg-slate-50 border border-border rounded-xl font-bold text-xs resize-none font-sans" rows={2} placeholder={t("erp_444")} value={barcodeExplanation} onChange={e => setBarcodeExplanation(e.target.value)} />
                        </div>
                      </div>}
                  </div>
                </div>}
            ...
            <div className={cn("border rounded-2xl overflow-hidden bg-white shadow-sm transition-all", activeAccordion === "E" ? "border-primary/40 ring-1 ring-primary/10" : "border-border")}>
              <button type="button" onClick={() => setActiveAccordion(activeAccordion === "E" ? null : "E")} className="w-full px-4 py-3 flex items-center justify-between bg-slate-50 hover:bg-slate-100/80 transition-colors cursor-pointer">
                <div className="flex items-center gap-2">
              ...
              {activeAccordion === "E" && <div className="p-4 border-t border-border space-y-4 animate-in fade-in duration-200">
                  {/* Font Scale Slider */}
                  <div className="bg-slate-50 border border-slate-200/60 p-3 rounded-2xl space-y-1.5 shrink-0">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-black text-gray-700 flex items-center gap-1">
                    ...
                    <input type="range" min="0.6" max="2.0" step="0.05" value={groupFontScales.E} onChange={e => updateGroupFontScale("E", parseFloat(e.target.value))} className="w-full h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-purple-600" />
                  </div>
                  <div className="flex items-center justify-between pb-1.5 border-b border-dashed border-slate-100">
                    <label className="flex items-center gap-2 cursor-pointer font-black text-[10.5px] text-gray-700">
                      <input type="checkbox" checked={showNutrition} onChange={e => setShowNutrition(e.target.checked)} className="w-3.5 h-3.5 accent-primary rounded border-border" />
                      <span>{t("erp_660")}</span>
                    </label>
                  </div>

                  {showNutrition && <div className="space-y-4 animate-in fade-in duration-150">
                      {/* Servings Info */}
                      <div className="grid grid-cols-2 gap-3 bg-slate-50 p-3 border border-slate-200/50 rounded-2xl">
                        <div className="space-y-1">
                          ...
                          <input type="number" className="w-full px-2.5 py-1.5 bg-white border border-border rounded-xl font-mono text-xs text-right font-bold" value={portionSize} onChange={e => setPortionSize(e.target.value)} />
                        </div>
                        <div className="space-y-1">
                          <span className="text-[9px] font-bold text-gray-500">
                            {t("erp_662")}
                          </span>
                          <input type="number" step="any" className="w-full px-2.5 py-1.5 bg-white border border-border rounded-xl font-mono text-xs text-right font-bold" value={portionsPerPkg} onChange={e => setPortionsPerPkg(e.target.value)} />
                        </div>
                      </div>
                      ...
                      return <div key={nut.key} className="p-2 bg-slate-50/50 border border-slate-200/50 rounded-xl space-y-1">
                                <div className="flex justify-between items-center text-[9px] font-bold text-gray-700">
                                  <span>{nut.label}</span>
                                </div>
                                <div className="grid grid-cols-12 gap-1 items-center">
                                  <div className="col-span-5">
                                    <input type="number" className="w-full px-2 py-1 bg-white border border-border rounded-lg font-mono text-[10.5px] text-right font-bold" value={nut.val} onChange={e => nut.setter(e.target.value)} />
                                  </div>
                                  <div className="col-span-4">
                                    <select className="w-full px-1 py-1 bg-white border border-border rounded-lg text-[9px] font-bold text-gray-700 outline-none" value={cfg.decimals} onChange={e => updateNutritionConfig(nut.key, {
                              decimals: parseInt(e.target.value)
                                  ...
                                  <div className="col-span-3 flex items-center gap-0.5">
                                    ...
                                    <input type="number" min="1" max="20" className="w-full px-1 py-0.5 bg-white border border-border rounded text-[9px] font-mono text-center font-bold text-gray-700" value={cfg.maxLength} onChange={e => updateNutritionConfig(nut.key, {
                              maxLength: parseInt(e.target.value) || 8
                            })} />
                                  </div>
                                </div>
                              </div>;
            ...
            <div className={cn("border rounded-2xl overflow-hidden bg-white shadow-sm transition-all", activeAccordion === "L" ? "border-primary/40 ring-1 ring-primary/10" : "border-border")}>
              <button type="button" onClick={() => setActiveAccordion(activeAccordion === "L" ? null : "L")} className="w-full px-4 py-3 flex items-center justify-between bg-slate-50 hover:bg-slate-100/80 transition-colors cursor-pointer">
                <div className="flex items-center gap-2">
              ...
              {activeAccordion === "L" && <div className="p-4 border-t border-border space-y-4 animate-in fade-in duration-200">
                  <div className="flex gap-2">
                    <button type="button" onClick={handleAddLine} className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-black text-xs shadow-sm transition-all active:scale-95 flex items-center justify-center gap-1.5 cursor-pointer">
                  ...
                  {customLines.length === 0 ? <div className="text-center py-6 border border-dashed border-slate-200 rounded-2xl text-slate-400 font-bold text-xs">
                      {t("erp_683")}
                      <br />
                      {t("erp_684")}
                    </div> : <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
                      {customLines.map((line, index) => <div key={line.id} className="p-3 bg-slate-50 border border-slate-200 rounded-2xl space-y-3 relative group/item">
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] font-black text-slate-700 font-sans">
                              {t("erp_685")}
                              {index + 1} (
                              {line.type === "horizontal" ? i18n.t("erp_686") : i18n.t("erp_687")}
                              )
                            </span>
                            <button type="button" onClick={() => handleDeleteLine(line.id)} className="text-red-500 hover:text-red-700 p-1 rounded hover:bg-red-50 transition-colors cursor-pointer">
                              <Trash2 className="w-3.5 h-3.5" />
                            </button>
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            ...
                            <div className="space-y-1">
                              ...
                              <div className="flex rounded-lg border border-slate-200 overflow-hidden bg-white p-0.5">
                                <button type="button" onClick={() => handleUpdateLine(line.id, {
                          type: "horizontal"
                        })} className={cn("flex-1 py-1 text-[9px] font-black rounded cursor-pointer", line.type === "horizontal" ? "bg-slate-900 text-white" : "text-slate-600 hover:bg-slate-50")}>
                                  {t("erp_686")}
                                </button>
                                <button type="button" onClick={() => handleUpdateLine(line.id, {
                          type: "vertical"
                        })} className={cn("flex-1 py-1 text-[9px] font-black rounded cursor-pointer", line.type === "vertical" ? "bg-slate-900 text-white" : "text-slate-600 hover:bg-slate-50")}>
                                  {t("erp_687")}
                                </button>
                              </div>
                            </div>
                            ...
                            <div className="space-y-1">
                              ...
                              <select className="w-full px-2 py-1 bg-white border border-slate-200 rounded-lg text-[9.5px] font-bold text-slate-700 outline-none" value={line.style} onChange={e => handleUpdateLine(line.id, {
                        style: e.target.value as any
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            ...
                            <div className="space-y-1">
                              ...
                              <select className="w-full px-2 py-1 bg-white border border-slate-200 rounded-lg text-[9.5px] font-bold text-slate-700 outline-none" value={line.thickness} onChange={e => handleUpdateLine(line.id, {
                        thickness: e.target.value
                            ...
                            <div className="space-y-1">
                              ...
                              <input type="range" min="2" max="100" className="w-full h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-indigo-600" value={line.length} onChange={e => handleUpdateLine(line.id, {
                        length: parseInt(e.target.value) || 50
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            ...
                            <div className="space-y-1">
                              ...
                              <input type="range" min="0" max="100" className="w-full h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-indigo-600" value={line.left} onChange={e => handleUpdateLine(line.id, {
                        left: parseInt(e.target.value) || 0
                            ...
                            <div className="space-y-1">
                              ...
                              <input type="range" min="0" max="100" className="w-full h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-indigo-600" value={line.top} onChange={e => handleUpdateLine(line.id, {
        ...
        <div className="lg:col-span-7 flex flex-col items-center justify-start p-8 bg-[#D1D4D9] rounded-3xl min-h-[75vh] relative overflow-hidden border border-border shadow-inner print:bg-transparent print:border-none print:shadow-none print:p-0">
          ...
          return <div className={cn("w-full flex flex-col items-center justify-start min-h-0 print:overflow-visible print:max-h-none print:py-0 print:pr-0", previewContinuous ? "overflow-y-auto max-h-[62vh] py-8 pr-12 scrollbar-none print:pr-0 print:py-0" : "justify-center py-4")}>
                {/* Simulated Continuous Backing Paper Roll Wrapper */}
                <div className={cn("continuous-backing-paper flex flex-col items-center relative transition-all duration-300", previewContinuous ? "shadow-xl border-x-[1.5px] border-dashed border-[#D2C095] pt-[15mm] pb-[15mm] print:shadow-none print:border-none print:pt-0 print:pb-0" : "pt-0 pb-0")} style={previewContinuous ? {
              ...
              background: "linear-gradient(to right, #F3E6C4 0%, #FAF1D7 8%, #FDF7E7 50%, #FAF1D7 92%, #F3E6C4 100%)"
                  ...
                  {previewContinuous && <div className="absolute right-[-45px] top-0 bottom-0 w-[40px] flex flex-col items-start justify-center print:hidden text-slate-500 font-mono select-none">
                  ...
                  height: `${activeHeight}mm`
                      ...
                      {upperGap > 0 && <div className="absolute flex flex-col items-center justify-center animate-in fade-in duration-200" style={{
                  top: `35mm`,
                  height: `${upperGap}mm`
                }}>
                      ...
                      {lowerGap > 0 && <div className="absolute flex flex-col items-center justify-center animate-in fade-in duration-200" style={{
                  ...
                  {previewContinuous && <div className="opacity-[0.12] border-2 border-black border-dashed rounded-none bg-white flex flex-col justify-between p-2 pointer-events-none select-none shrink-0 print:hidden animate-in fade-in duration-300" style={{
                ...
                overflow: "hidden"
                  ...
                  {/* 2. Main Active Label */}
                  <div id="printable-label" style={getLabelStyle()} className={cn("relative shadow-md print:shadow-none shrink-0 transition-shadow rounded-none overflow-hidden select-none", useCustomLayout && "border-slate-800")}>
                    {/* Dotted Grid Background Overlay for Drag Alignment - Hidden in Print */}
                    {useCustomLayout && <div className="absolute inset-0 pointer-events-none print:hidden z-0" style={{
                  ...
                  opacity: 0.15
                    ...
                    {customLines.map(line => <div key={line.id} className={cn("absolute pointer-events-auto", useCustomLayout ? "hover:bg-indigo-50/20 group/line" : "pointer-events-none")} style={{
                  left: `${line.left}%`,
                  top: `${line.top}%`,
                  ...
                  zIndex: 15
                        ...
                        {useCustomLayout && <>
                            ...
                            <div className="absolute top-[-16px] left-0 bg-slate-900 text-white font-mono text-[7px] px-1.5 py-0.5 rounded shadow opacity-0 group-hover/line:opacity-100 transition-opacity z-30 print:hidden flex items-center gap-1 select-none whitespace-nowrap">
                              ...
                              <button type="button" onClick={() => handleDeleteLine(line.id)} className="text-red-400 hover:text-red-600 font-bold ml-1 cursor-pointer">
                                ✕
                              </button>
                            </div>
                            {/* Drag Trigger Overlay */}
                            <div onMouseDown={e => handleLineDragStart(e, line.id)} className="absolute inset-0 cursor-move print:hidden z-10 bg-transparent" style={{
                      padding: "4px",
                      margin: "-4px"
                    }} />
                            {/* Resize Handle (circle anchor) */}
                            <div onMouseDown={e => handleLineResizeStart(e, line.id)} className={cn("absolute bg-indigo-600 rounded-full w-2.5 h-2.5 border border-white cursor-pointer z-20 print:hidden shadow-sm hover:scale-125 transition-transform", line.type === "horizontal" ? "right-0 top-1/2 -translate-y-1/2 translate-x-1/2" : "bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2")} />
                          </>}
                      </div>)}

                    {/* Drag-and-Resize Active Group Coordinates Overlay Tooltip - Hidden in Print */}
                    {useCustomLayout && (draggingGroup || resizingGroup) && <div className="absolute top-2 left-2 bg-indigo-900/90 text-white font-mono font-bold text-[7.5px] px-2 py-1 rounded shadow-md z-30 pointer-events-none animate-pulse print:hidden border border-indigo-700">
                        {draggingGroup ? `DRAGGING [Group ${draggingGroup}]` : `RESIZING [Group ${resizingGroup}]`}{" "}
                        : L:{" "}
                        {groupLayouts[draggingGroup || resizingGroup || "A"].left}
                        % T:{" "}
                        {groupLayouts[draggingGroup || resizingGroup || "A"].top}
                        % W:{" "}
                        {groupLayouts[draggingGroup || resizingGroup || "A"].width}
                        % H:{" "}
                        {groupLayouts[draggingGroup || resizingGroup || "A"].height}
                        %
                      </div>}
                ...
                <div className="w-full h-full relative z-10">
                        ...
                        <div className={cn("absolute flex flex-col justify-start overflow-hidden border border-transparent hover:border-slate-300 transition-colors bg-white/95 rounded p-1 group/item", draggingGroup === "B" && "border-indigo-500 bg-indigo-50/10 z-20 shadow-md", resizingGroup === "B" && "border-purple-500 bg-purple-50/10 z-20 shadow-md")} style={{
                    ...
                    height: `${groupLayouts.B.height}%`
                        ...
                        ...groupLayouts.B
                          ...
                          <div className="w-full h-full flex items-center justify-between pt-3 pb-1 min-h-0 text-black px-1">
                            {labelSize !== "70x50" ? <>
                                ...
                                {showBranding && <div className="flex items-center gap-1.5 overflow-hidden shrink-0">
                                    <div className="w-5 h-5 shrink-0 flex items-center justify-center border border-black rounded p-0.5 bg-white">
                                      {renderLabelLogo()}
                                    </div>
                                    <div className="flex flex-col text-left min-w-0">
                                      <span className="font-extrabold tracking-wide leading-none text-black truncate" style={{
                              fontSize: `${7.5 * groupFontScales.B}pt`
                            }}>
                                        {brandNameZh}
                                      </span>
                                      <span className="font-bold tracking-wider text-black uppercase leading-none truncate mt-0.5" style={{
                              fontSize: `${4.5 * groupFontScales.B}pt`
                                ...
                                <div className="text-right flex-1 pl-2 overflow-hidden leading-tight flex flex-col justify-center">
                                  {showProductZh && <h1 className="font-black leading-none tracking-tight text-black break-words" style={{
                            fontSize: `${10 * groupFontScales.B}pt`
                          }}>
                                      {productZh}
                                    </h1>}
                                  {showProductEn && <span className="font-bold uppercase text-black block leading-none truncate mt-0.5" style={{
                      ...
                      <div className="w-full flex items-center justify-between min-h-0 leading-none">
                                {showProductZh && <span className="font-black text-black text-left truncate flex-1 pr-1.5" style={{
                          fontSize: `${9 * groupFontScales.B}pt`
                        }}>
                                    {productZh}
                                  </span>}
                                {showBranding && <span className="font-bold text-black text-right shrink-0" style={{
                      ...
                      setDragStartLayout({
                        ...
                        <div className={cn("absolute flex flex-col justify-start overflow-hidden border border-transparent hover:border-slate-300 transition-colors bg-white/95 rounded p-1 group/item", draggingGroup === "F" && "border-indigo-500 bg-indigo-50/10 z-20 shadow-md", resizingGroup === "F" && "border-purple-500 bg-purple-50/10 z-20 shadow-md")} style={{
                    ...
                    height: `${groupLayouts.F.height}%`
                      ...
                      setDragStartLayout({
                        ...groupLayouts.F
                          ...
                          <div className="w-full h-full flex flex-col justify-between pt-3 pb-1 min-h-0 text-black">
                            {/* Ingredients */}
                            {showIngredients && <div className="flex-1 flex flex-col justify-start min-h-0 overflow-hidden text-black leading-tight">
                                <span className="font-black bg-black text-white px-1 py-[0.2mm] rounded-[0.2mm] self-start leading-none shrink-0 mb-0.5" style={{
                          fontSize: `${5.8 * groupFontScales.F}pt`
                        }}>
                                  {t("erp_705")}
                                </span>
                                <p className="font-semibold text-justify word-break break-all text-black leading-[1.25] overflow-y-auto" style={{
                          fontSize: `${5.8 * groupFontScales.F}pt`
                        }}>
                            ...
                            <div className="shrink-0 space-y-0.5 mt-1 leading-none">
                              {showNotReadyToEat && <div className="font-black text-center text-white bg-black border border-black py-0.5 rounded-[0.2mm]" style={{
                          fontSize: `${5.5 * groupFontScales.F}pt`
                        }}>
                                  ⚠️ {notReadyToEatText}
                                </div>}
                              {showAllergens && allergenWarning && <div className="font-black text-black bg-white p-0.5 rounded-[0.2mm] border border-black border-dashed leading-tight" style={{
                      ...
                      setDragStartLayout({
                        ...
                        <div className={cn("absolute flex flex-col justify-start overflow-hidden border border-transparent hover:border-slate-300 transition-colors bg-white/95 rounded p-1 group/item", draggingGroup === "E" && "border-indigo-500 bg-indigo-50/10 z-20 shadow-md", resizingGroup === "E" && "border-purple-500 bg-purple-50/10 z-20 shadow-md")} style={{
                    ...
                    height: `${groupLayouts.E.height}%`
                      ...
                      setDragStartLayout({
                        ...groupLayouts.E
                          ...
                          <div className="w-full h-full flex flex-col justify-end pt-3 pb-1 min-h-0 text-black">
                            {showNutrition ? <div className="flex flex-col justify-end text-black shrink-0 border border-black p-0.5 bg-white">
                                <div className="font-black text-center border-b border-black pb-0.5 tracking-wider leading-none" style={{
                          fontSize: `${6.2 * groupFontScales.E}pt`
                        }}>
                                  {t("erp_708")}
                                </div>
                                <div className="font-bold text-left py-0.5 border-b border-black leading-tight" style={{
                          fontSize: `${4.8 * groupFontScales.E}pt`
                                ...
                                <table className="w-full text-center border-collapse mt-0.5" style={{
                          fontSize: `${5 * groupFontScales.E}pt`
                        }}>
                                  <thead>
                                    <tr className="border-b border-black" style={{
                              ...
                              return <tr key={idx} className="text-black font-black">
                              ...
                              </div> : <div className="w-full text-center text-[6pt] text-slate-400 font-bold border border-slate-200 border-dashed py-4 rounded">
                                {t("erp_718")}
                              </div>}
                          </div>
                      ...
                      setDragStartLayout({
                        ...
                        <div className={cn("absolute flex flex-col justify-start overflow-hidden border border-transparent hover:border-slate-300 transition-colors bg-white/95 rounded p-1 group/item", draggingGroup === "C" && "border-indigo-500 bg-indigo-50/10 z-20 shadow-md", resizingGroup === "C" && "border-purple-500 bg-purple-50/10 z-20 shadow-md")} style={{
                    ...
                    height: `${groupLayouts.C.height}%`
                      ...
                      setDragStartLayout({
                        ...groupLayouts.C
                          ...
                          <div className="w-full h-full flex flex-col justify-around pt-3 pb-1 min-h-0 text-black leading-tight">
                            <div className="font-extrabold space-y-[0.3mm] text-black" style={{
                        fontSize: `${5.8 * groupFontScales.C}pt`
                      }}>
                              {showNetWeight && <div className="flex justify-between">
                                  <span>{t("erp_720")}</span>
                                  <span className="font-mono">{netWeight}</span>
                                </div>}
                              {showStorage && <div className="flex justify-between">
                                  <span>{t("erp_721")}</span>
                              ...
                              {showExpiry && <>
                                  ...
                                  <div className="flex justify-between font-black">
                                    <span>{t("erp_723")}</span>
                                    <span className="font-mono">
                                      {expiryOption === "printed" ? i18n.t("erp_724") : expiryDate}
                                    </span>
                                  </div>
                                </>}
                              {showOrigin && originCountry && <div className="flex justify-between">
                                  <span>{t("erp_725")}</span>
                                  <span className="font-extrabold">
                                    {originCountry}
                                  </span>
                      ...
                      setDragStartLayout({
                        ...
                        <div className={cn("absolute flex flex-col justify-start overflow-hidden border border-transparent hover:border-slate-300 transition-colors bg-white/95 rounded p-1 group/item", draggingGroup === "A" && "border-indigo-500 bg-indigo-50/10 z-20 shadow-md", resizingGroup === "A" && "border-purple-500 bg-purple-50/10 z-20 shadow-md")} style={{
                    ...
                    height: `${groupLayouts.A.height}%`
                      ...
                      setDragStartLayout({
                        ...groupLayouts.A
                          ...
                          {/* Content Container */}
                          <div className="w-full h-full flex flex-col justify-between pt-3 pb-1 min-h-0 text-black">
                            {showReheating ? <div className="w-full h-full flex flex-col justify-between overflow-hidden">
                            ...
                            steps: panSteps,
                            show: showPan
                          }].filter(m => m.show).map((m, idx) => <div key={idx} className="flex flex-col text-black">
                                        <strong style={{
                              ...
                              </div> : <div className="w-full text-center text-[5.5pt] text-slate-400 font-bold border border-slate-200 border-dashed py-2 rounded">
                                {t("erp_727")}
                              </div>}
                          </div>
                      ...
                      setDragStartLayout({
                        ...
                        <div className={cn("absolute flex flex-col justify-start overflow-hidden border border-transparent hover:border-slate-300 transition-colors bg-white/95 rounded p-1 group/item", draggingGroup === "D" && "border-indigo-500 bg-indigo-50/10 z-20 shadow-md", resizingGroup === "D" && "border-purple-500 bg-purple-50/10 z-20 shadow-md")} style={{
                      ...
                      setDragStartLayout({
                        ...groupLayouts.D
                          ...
                          <div className="w-full h-full flex items-center justify-between pt-3 pb-0.5 min-h-0 text-black leading-tight gap-1.5">
                            {/* Responsible party info */}
                            {showResponsible && <div className="text-black font-semibold flex-1 min-w-0" style={{
                        fontSize: `${5.2 * groupFontScales.D}pt`
                      }}>
                                {showAddress && <div className="flex gap-[0.2mm]">
                                ...
                                {showPhone && <div className="flex gap-[0.2mm]">
                                    <span className="font-black shrink-0">
                                      {t("erp_730")}
                                    </span>
                                    <span className="font-mono">
                                ...
                                {showManufacturer && <div className="flex gap-[0.2mm] text-slate-600 mt-0.5" style={{
                          fontSize: `${5 * groupFontScales.D}pt`
                            ...
                            {/* Barcode area */}
                            {showBarcode && <div className="flex items-center gap-1.5 shrink-0 border-l border-dashed border-black pl-1.5 py-0.5">
                                <div className="p-0.5 bg-white border border-black shrink-0 flex items-center justify-center">
                                  <QRCodeSVG value={barcodeText || "https://shutterorder.com"} size={24} level="M" fgColor="#000000" bgColor="#ffffff" />
                                </div>
                                <span className="font-sans font-black leading-tight text-black whitespace-pre-line text-left" style={{
                ...
                <div className="w-full h-full flex flex-col justify-between relative z-10 text-black">
                        ...
                        {labelSize !== "70x50" ? <div className="w-full border-b-[0.8mm] border-black pb-[1.5mm] flex justify-between items-center shrink-0">
                            {showBranding && <div className="flex items-center gap-[2.5mm] overflow-hidden">
                                <div style={{
                        width: "12mm",
                        height: "12mm"
                                ...
                                <div className="flex flex-col justify-center text-black">
                                  <span className="font-extrabold tracking-wide leading-none text-black" style={{
                          fontSize: `${12 * groupFontScales.B}pt`
                        }}>
                                    {brandNameZh}
                                  </span>
                                  <span className="font-bold tracking-widest text-black uppercase mt-0.5 leading-none" style={{
                          fontSize: `${5.5 * groupFontScales.B}pt`
                        }}>
                                    {brandNameEn}
                                  </span>
                            ...
                            <div className="text-right flex-1 pl-[3mm] overflow-hidden">
                              {showProductZh && <h1 className="font-black tracking-tight leading-none text-black break-words" style={{
                        fontSize: `${13 * groupFontScales.B}pt`
                      }}>
                                  {productZh}
                                </h1>}
                              {showProductEn && <span className="font-extrabold tracking-wide uppercase text-black block mt-1.5 leading-none truncate" style={{
                  ...
                  <div className="w-full border-b-[0.5mm] border-black pb-[0.5mm] flex justify-between items-end shrink-0">
                            <span className="font-black leading-none text-black" style={{
                      fontSize: `${9 * groupFontScales.B}pt`
                    }}>
                              {productZh}
                            </span>
                            <span className="font-bold text-black leading-none" style={{
                      fontSize: `${5.5 * groupFontScales.B}pt`
                    }}>
                              {brandNameZh}
                            </span>
                  ...
                  <div style={hasActiveReheating ? {
                    gridTemplateColumns: "1.1fr 1fr"
                  } : undefined} className={cn("flex-1 min-h-0 text-black", hasActiveReheating ? "grid gap-[3mm] py-[2mm] grid-cols-[1.1fr_1fr]" : "py-[2mm] flex flex-col justify-between")}>
                            {/* Left Column: Reheating steps */}
                            {hasActiveReheating && <div className="border-r-[0.3mm] border-black pr-[2mm] flex flex-col justify-between gap-[2.5mm] min-h-0 overflow-hidden text-black h-full">
                          ...
                          steps: panSteps,
                          show: showPan
                        }].filter(m => m.show).map((m, idx) => <div key={idx} className="flex flex-col gap-[0.5mm] text-black">
                                        <strong style={{
                            ...
                            <div className="flex flex-col justify-between gap-[2.5mm] min-h-0 h-full text-black">
                              {/* Ingredients */}
                              {showIngredients && <div className="flex flex-col gap-[1mm] min-h-0 overflow-hidden text-black flex-1 justify-start">
                                  <span className="font-black bg-black text-white px-[1.5mm] py-[0.3mm] rounded-[0.3mm] self-start leading-none shrink-0 mb-1" style={{
                          fontSize: `${7 * groupFontScales.F}pt`
                        }}>
                                    {t("erp_705")}
                                  </span>
                                  <p className="font-semibold text-justify word-break break-all text-black pl-0.5 leading-[1.3] overflow-y-auto" style={{
                          fontSize: `${6.6 * groupFontScales.F}pt`
                              ...
                              {showNotReadyToEat && <div className="font-black text-center text-white bg-black py-0.5 rounded-[0.3mm] shrink-0 my-0.5" style={{
                        fontSize: `${6.2 * groupFontScales.F}pt`
                              ...
                              {/* Expiry / Weight */}
                              {!shouldMoveInfoToBottomLeft && (showNetWeight || showStorage || showExpiry || showOrigin) && <div className="leading-[1.35] font-bold space-y-[0.6mm] border-t-[0.2mm] border-dashed border-black pt-2 text-black shrink-0" style={{
                        fontSize: `${6.8 * groupFontScales.C}pt`
                      }}>
                                    {showNetWeight && <div className="flex justify-between text-black">
                                        <span>{t("erp_720")}</span>
                                        <span className="font-mono">
                                          {netWeight}
                                        </span>
                                      </div>}
                                    {showStorage && <div className="flex justify-between text-black">
                                        <span>{t("erp_721")}</span>
                                        <span>{storageCondition}</span>
                                      </div>}
                                    {showExpiry && <>
                                    ...
                                    {showOrigin && originCountry && <div className="flex justify-between text-black">
                                        <span>{t("erp_725")}</span>
                                        <span className="font-extrabold">
                                          {originCountry}
                                        </span>
                              ...
                              {/* Allergen alert */}
                              {showAllergens && allergenWarning && <div className="text-[6pt] font-black text-black bg-white p-[1.2mm] rounded-[0.5mm] border-[0.25mm] border-black border-dashed leading-[1.2] shrink-0">
                                  {t("erp_732")}
                                  {allergenWarning}
                                </div>}
                            </div>
                  ...
                  <div className="flex-1 py-[1mm] flex flex-col justify-between text-black leading-[1.2] font-extrabold" style={{
                    fontSize: `${5.8 * groupFontScales.F}pt`
                  }}>
                            {showIngredients && <p className="text-justify word-break break-all text-black font-semibold" style={{
                      fontSize: `${5.8 * groupFontScales.F}pt`
                    }}>
                            ...
                            {showNotReadyToEat && <div className="font-black text-center text-white bg-black py-0.5 rounded-[0.2mm] shrink-0 my-0.5" style={{
                      fontSize: `${5.2 * groupFontScales.F}pt`
                    }}>
                                ⚠️ {notReadyToEatText}
                              </div>}

                            <div className="grid grid-cols-2 gap-[2mm] border-t-[0.1mm] border-black pt-[1mm] mt-[0.5mm]" style={{
                      fontSize: `${5.8 * groupFontScales.C}pt`
                    }}>
                              <div>
                                {showNetWeight && <p>
                              ...
                              <div className="text-right">
                                {showExpiry && <p className="font-black text-black">
                                    {t("erp_736")}
                                    {expiryOption === "printed" ? i18n.t("erp_737") : expiryDate}
                                  </p>}
                                {showOrigin && originCountry && <p>
                                    <span className="font-black">
                                      {t("erp_738")}
                                    </span>
                                    {originCountry}
                            ...
                            {showAllergens && <p className="font-black text-black bg-white border-[0.1mm] border-dashed border-black p-0.5 mt-0.5" style={{
                      fontSize: `${5 * groupFontScales.F}pt`
                    }}>
                                {t("erp_739")}
                                {allergenWarning.replace(i18n.t("erp_740"), "").replace(i18n.t("erp_741"), "")}
                              </p>}
                          </div>}

                        {/* 3. Bottom Row: Nutrition facts table & corporate details & QR code */}
                        {labelSize !== "70x50" ? <div style={showNutrition ? {
                    gridTemplateColumns: "1.1fr 1fr"
                  } : undefined} className={cn("w-full border-t-[0.8mm] border-black pt-[2mm] shrink-0 text-black", showNutrition ? "grid gap-[3mm] items-end grid-cols-[1.1fr_1fr]" : "block")}>
                            {/* Bottom Left: Corporate Details + Barcode Area */}
                            <div className={cn("overflow-hidden text-black w-full", showNutrition ? "space-y-[2mm]" : "flex justify-between items-end gap-[4mm] border-b-[0.1mm] border-dashed border-slate-300 pb-1")}>
                              {shouldMoveInfoToBottomLeft && (showNetWeight || showStorage || showExpiry || showOrigin) && <div className="leading-[1.35] font-bold space-y-[0.6mm] border-b-[0.2mm] border-dashed border-black pb-2 mb-2 text-black shrink-0" style={{
                        fontSize: `${6.8 * groupFontScales.C}pt`
                      }}>
                                    {showNetWeight && <div className="flex justify-between text-black">
                                        <span>{t("erp_720")}</span>
                              ...
                              {showResponsible && <div className={cn("leading-[1.3] text-black font-semibold", showNutrition ? "" : "flex-1")} style={{
                        fontSize: `${6.2 * groupFontScales.D}pt`
                      }}>
                                  {showAddress && <div className="flex gap-[0.5mm]">
                                      <span className="font-black shrink-0">
                                        {t("erp_729")}
                                      </span>
                                      <span>{companyAddress}</span>
                                    </div>}
                                  {showPhone && <div className="flex gap-[0.5mm]">
                                      <span className="font-black shrink-0">
                                        {t("erp_730")}
                                      </span>
                                      <span className="font-mono">
                                  ...
                                  {showOrigin && !shouldMoveInfoToBottomLeft && <div className="flex gap-[0.5mm]">
                                        <span className="font-black shrink-0">
                                          {t("erp_725")}
                                        </span>
                                  ...
                                  {showManufacturer && <div className="flex gap-[0.5mm] mt-0.5 text-black" style={{
                          fontSize: `${5.8 * groupFontScales.D}pt`
                              ...
                              {showBarcode && <div className={cn("flex items-center gap-[2mm] shrink-0", showNutrition ? "border-t-[0.2mm] border-dashed border-black pt-[1.5mm]" : "border-l-[0.2mm] border-dashed border-black pl-[4mm] py-0.5")}>
                                  <div className="p-0.5 bg-white border border-black shrink-0 flex items-center justify-center">
                                    <QRCodeSVG value={barcodeText || "https://shutterorder.com"} size={32} level="M" fgColor="#000000" bgColor="#ffffff" />
                                  </div>
                                  <span className="font-sans font-black leading-tight text-black whitespace-pre-line text-left" style={{
                          fontSize: `${5.2 * groupFontScales.D}pt`
                        }}>
                            ...
                            {showNutrition && <div className="flex flex-col justify-end text-black shrink-0 border border-black p-1 bg-white">
                                <div className="font-black text-center border-b-[0.25mm] border-black pb-0.5 tracking-[1mm] text-black leading-none" style={{
                        fontSize: `${7 * groupFontScales.E}pt`
                      }}>
                                  {t("erp_708")}
                                </div>
                                <div className="font-bold text-left py-1 leading-normal border-b-[0.15mm] border-black text-black" style={{
                        fontSize: `${5.5 * groupFontScales.E}pt`
                                ...
                                <table className="w-full text-center border-collapse mt-0.5 text-black" style={{
                        fontSize: `${5.8 * groupFontScales.E}pt`
                      }}>
                                  <thead>
                                    <tr className="border-b-[0.15mm] border-black text-black" style={{
                            fontSize: `${5 * groupFontScales.E}pt`
                          }}>
                            ...
                            return <tr key={idx} className="text-black font-black">
                                          <td className={cn("py-[0.2mm] text-left font-sans pl-[0.5mm] text-black", row.name.startsWith("  ") ? "pl-[2mm] font-semibold text-black" : "font-black text-black")}>
                                            {row.name.trim()}
                                          </td>
                  ...
                  <div className="w-full border-t-[0.4mm] border-black pt-[0.5mm] flex justify-between items-center text-black font-black shrink-0" style={{
                    fontSize: `${5.2 * groupFontScales.D}pt`
                  }}>
                            {showResponsible && <span className="leading-none truncate max-w-[48mm]">
                                {t("erp_743")}
                                {companyName}
                                {t("erp_744")}
                                {companyPhone}
                              </span>}
                            {showBarcode && <span className="font-mono leading-none shrink-0 font-black border-[0.1mm] border-black px-0.5 rounded-[0.2mm]" style={{
